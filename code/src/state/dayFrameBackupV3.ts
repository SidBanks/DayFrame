...
export type ProfilesV2BackupData = {
  surfaceVersion: 2;
  profiles: DayFrameSavedProfile[];
  quarantinedProfiles: unknown[];
};
export type PlanDecisionV1BackupData = {
  surfaceVersion: 1;
...
export type DayFrameBackupV3Data = {
...
export type DayFrameBackupV3 = {
  app: "DayFrame";
  surface: "backup";
  version: typeof DAYFRAME_BACKUP_V3_VERSION;
  exportedAt: string;
  data: DayFrameBackupV3Data;
};

export type BackupV3ValidationCategory =
  | "envelopeValidationFailure"
  | "activeValidationFailure"
  | "profilesValidationFailure"
  | "planDecisionValidationFailure"
  | "executionHistoryValidationFailure"
  | "historicalPlanValidationFailure";
export class DayFrameBackupV3ValidationError extends RangeError {
  constructor(
    public readonly category: BackupV3ValidationCategory,
    message: string,
  ) {
    super(message);
    this.name = "DayFrameBackupV3ValidationError";
  }
}

export function createDayFrameBackupV3(
  data: DayFrameBackupV3Data,
  exportedAt: string,
): DayFrameBackupV3 {
  return validateDayFrameBackupV3({
    app: "DayFrame",
    surface: "backup",
    version: 3,
    exportedAt,
...
export function validateDayFrameBackupV3(value: unknown): DayFrameBackupV3 {
  if (
    !record(value) ||
    !exact(value, ["app", "surface", "version", "exportedAt", "data"]) ||
    value.app !== "DayFrame" ||
    ...
    throw invalid("envelopeValidationFailure", "Backup V3 envelope is invalid.");
  const data = value.data;
  let active: ActiveV2BackupData;
  try {
    if (
      !record(data.active) ||
      !exact(data.active, ["surfaceVersion", "data"]) ||
      data.active.surfaceVersion !== 2
    )
      throw new Error();
    const checked = validateActiveV2({
      app: "DayFrame",
      surface: "active",
      version: 2,
      data: data.active.data,
    });
    active = { surfaceVersion: 2, data: checked.data };
  } catch {
    throw invalid("activeValidationFailure", "Backup V3 Active authority is invalid.");
  }
  let profiles: ProfilesV2BackupData;
  try {
    if (
      !record(data.profiles) ||
      !exact(data.profiles, ["surfaceVersion", "profiles", "quarantinedProfiles"]) ||
      data.profiles.surfaceVersion !== 2
    )
      throw new Error();
    const checked = validateDayFrameProfilesStorageV2({
      app: "DayFrame",
      surface: "profiles",
      version: 2,
    ...
    profiles = {
      surfaceVersion: 2,
      profiles: checked.profiles.sort((a, b) => a.id.localeCompare(b.id)),
      quarantinedProfiles: structuredClone(checked.quarantinedProfiles),
    };
  } catch {
    throw invalid("profilesValidationFailure", "Backup V3 Profiles authority is invalid.");
  }
  ...
  return {
    ...
    data: {
      active: { surfaceVersion: 2, data: cloneActiveSetup(active.data) },
      profiles,
      planDecisions,
      executionHistory,
      historicalPlan,
...
export function backupV3SemanticFingerprint(
  value: DayFrameBackupV3 | DayFrameBackupV3Data,
): string {
  const data =
    "data" in value
      ? validateDayFrameBackupV3(value).data
...
export function activeV2BackupData(value: DayFrameActiveV2): ActiveV2BackupData {
  const checked = validateActiveV2(value);
  return { surfaceVersion: 2, data: checked.data };
}
export function profilesV2BackupData(
  profiles: DayFrameSavedProfile[],
  quarantine: unknown[],
): ProfilesV2BackupData {
  const checked = createDayFrameProfilesStorageV2(profiles, quarantine);
  return {
    surfaceVersion: 2,
    profiles: checked.profiles.sort((a, b) => a.id.localeCompare(b.id)),
    quarantinedProfiles: structuredClone(checked.quarantinedProfiles),
...
function validatePlanDecisionData(value: unknown): PlanDecisionV1BackupData {
  try {
    if (
      !record(value) ||
      !exact(value, ["surfaceVersion", "decisions", "quarantinedDecisions"]) ||
      value.surfaceVersion !== 1 ||
      !Array.isArray(value.decisions) ||
      !Array.isArray(value.quarantinedDecisions)
    )
      throw new Error();
    const decisionsCheck = buildPlanDecisionRuntimeTarget({
      app: "DayFrame",
      surface: "planDecisions",
      version: 1,
      decisions: value.decisions,
    });
    if (decisionsCheck.status !== "valid" || decisionsCheck.target.quarantined.length)
      throw new Error();
    const quarantine = value.quarantinedDecisions
      .map((entry) => {
        if (
          !record(entry) ||
          !exact(entry, ["quarantineId", "reason", "raw"]) ||
          typeof entry.quarantineId !== "string" ||
    ...
    return {
      surfaceVersion: 1,
      decisions: decisionsCheck.target.decisions,
      quarantinedDecisions: quarantine,
    };
  } catch {
    throw invalid("planDecisionValidationFailure", "Backup V3 PlanDecision authority is invalid.");
  }
}
function validateExecutionData(value: unknown): ExecutionHistoryV1BackupData {
  ...
  if (
    ...
    JSON.stringify(checked.envelope.records.map((entry) => entry.id)) !==
      JSON.stringify(
        (value.records as unknown[]).map((entry) => (record(entry) ? entry.id : undefined)),
      ) ||
    checked.envelope.quarantinedComponents.length !==
      (value.quarantinedComponents as unknown[]).length
  )
    throw invalid(
      "executionHistoryValidationFailure",
...
function validateHistoricalData(value: unknown): HistoricalPlanV1BackupData {
  try {
    if (
      !record(value) ||
      !exact(value, ["surfaceVersion", "batches"]) ||
      value.surfaceVersion !== 1 ||
    ...
    for (const raw of value.batches) {
      const checked = validatePlanPublicationBatch(raw);
      if (checked.status !== "valid" || ids.has(checked.batch.id)) throw new Error();
      ids.add(checked.batch.id);
      for (const day of checked.batch.days) {
        const key = `${checked.batch.publishedAt}|${day.userDayDate}`;
        if (dayTimes.has(key)) throw new Error();
        dayTimes.add(key);
      }
