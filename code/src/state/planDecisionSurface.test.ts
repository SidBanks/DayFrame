...
const manual = { id: "manual", title: "Event", userDayDate: "2026-08-20" as const,
...
function manualTarget(store: ReturnType<typeof storeWithManual>) {
  const result = createDurableOccurrenceReference(createManualEventOccurrenceIdentity("manual"), store.getState());
...
describe("store-owned PlanDecision surface", () => {
  it("accepts, persists, notifies only decision subscribers, and supersedes cross-kind", () => {
    ...
    const stateListener = vi.fn(); const decisionListener = vi.fn();
    store.subscribe(stateListener); store.subscribePlanDecisions(decisionListener);
    const first = store.acceptPlanDecision({ kind: "omitOccurrence", target, payload: {},
      provenance: { source: "user" } });
    expect(first.status).toBe("accepted");
    expect(first.status === "accepted" && first.persistence.status).toBe("persisted");
    expect(stateListener).not.toHaveBeenCalled(); expect(decisionListener).toHaveBeenCalledTimes(1);
    const second = store.acceptPlanDecision({ kind: "setOccurrencePriority", target,
      payload: { priority: 1 }, provenance: { source: "suggestedFix", suggestedAction: "changePriority" } });
    expect(second.status).toBe("accepted");
    expect(store.getPlanDecisions()).toHaveLength(1);
    expect(store.getPlanDecisions()[0]).toMatchObject({ id: decisionId(2), kind: "setOccurrencePriority" });
  });
  ...
  it("keeps runtime authority after write failure and retries the exact desired record", () => {
    ...
    const setItem = vi.spyOn(storage, "setItem").mockImplementationOnce(() => { throw new Error("write"); });
    const result = store.acceptPlanDecision({ kind: "omitOccurrence", target, payload: {}, provenance: { source: "user" } });
  ...
  it("quarantines invalid, duplicate-ID, and duplicate-target entries deterministically", () => {
    const store = storeWithManual(); const target = manualTarget(store);
    const valid = { version: 1, id: decisionId(1), kind: "omitOccurrence", target, payload: {},
  ...
  it("protects malformed whole-surface input and requires source-safe recovery", () => {
    ...
    expect(store.acceptPlanDecision({ kind: "omitOccurrence", target: manualTarget(store), payload: {},
      provenance: { source: "user" } })).toEqual({ status: "rejected", reason: "protectedDecisionIngress" });
  ...
  it("retains session removal after persistence failure and retries the absent decision", () => {
    ...
    setItem.mockRestore();
    expect(store.retryPlanDecisionPersistence()).toMatchObject({ status: "attempted", persistence: { status: "persisted" } });
