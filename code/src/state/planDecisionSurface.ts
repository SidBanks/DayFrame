...
export type QuarantinedPlanDecision = {
  quarantineId: string;
  reason: "invalidDecision" | "unsupportedDecisionVersion" | "unsupportedTargetVersion" |
    "duplicateDecisionId" | "conflictingTarget";
  raw: unknown;
};
...
export type PlanDecisionIngressStatus =
  ...
  | { status: "recoveryRequired"; reason: "readFailure" | "corruptJson" |
      "invalidEnvelope" | "unsupportedVersion"; sourcePreserved: boolean };

export type AcceptPlanDecisionResult =
  | { status: "accepted"; decision: PlanDecisionV1; decisions: PlanDecisionV1[];
      persistence: PersistenceWriteOutcome }
  | { status: "rejected"; reason: "protectedDecisionIngress" | "invalidTarget" |
      "unsupportedTargetVersion" | "targetSourceMissing" | "targetLifetimeMismatch" |
      "targetOccurrenceMissing" | "invalidPayload" | "unsupportedDecisionKind" |
      "allocationFailure" };

export type RemovePlanDecisionResult =
  | { status: "removed"; decisionId: PlanDecisionId; decisions: PlanDecisionV1[];
      persistence: PersistenceWriteOutcome }
...
export function createPlanDecisionSurface(options: {
  ...
  function notifyDecisions(): void { for (const listener of decisionListeners) listener(getPlanDecisions()); }
  ...
  function persist(): PersistenceWriteOutcome {
    ...
    let raw: string;
    try { raw = serialize(envelope(desired)); }
  ...
  function acceptPlanDecision(input: AcceptPlanDecisionInput): AcceptPlanDecisionResult {
    if (ingress.status === "recoveryRequired") return { status: "rejected", reason: "protectedDecisionIngress" };
    let id: PlanDecisionId;
    try {
      id = allocate();
      if (!isPlanDecisionId(id) || decisions.some((decision) => decision.id === id)) throw new Error("id");
    } catch { return { status: "rejected", reason: "allocationFailure" }; }
    const candidate = { ...input, version: PLAN_DECISION_VERSION, id, acceptedAt: now() } as PlanDecisionV1;
    const validation = validatePlanDecision(candidate);
    if (validation.status === "unsupportedTargetVersion") return { status: "rejected", reason: "unsupportedTargetVersion" };
    if (validation.status !== "valid") {
      const kind = (input as { kind?: unknown }).kind;
      return { status: "rejected", reason:
        !["placeOccurrence", "omitOccurrence", "setOccurrenceDuration", "setOccurrencePriority"].includes(String(kind))
          ? "unsupportedDecisionKind" : "invalidPayload" };
    }
    const resolution = resolveDurableOccurrenceReference(validation.decision.target, options.getAuthoredSetup());
    if (resolution.status !== "resolved") {
      const reasons = { sourceMissing: "targetSourceMissing", lifetimeMismatch: "targetLifetimeMismatch",
        occurrenceMissing: "targetOccurrenceMissing", invalidReference: "invalidTarget",
        unsupportedVersion: "unsupportedTargetVersion" } as const;
      return { status: "rejected", reason: reasons[resolution.status] };
    }
    const key = getPlanDecisionTargetKey(validation.decision.target);
    decisions = [...decisions.filter((decision) => getPlanDecisionTargetKey(decision.target) !== key),
      validation.decision].sort(compareDecisions);
    desired = cloneEntries(decisions, quarantined);
    const persistence = persist();
    options.onAuthorityChanged?.();
    notifyDecisions();
    return { status: "accepted", decision: clonePlanDecision(validation.decision),
      decisions: getPlanDecisions(), persistence };
  }

  function removePlanDecision(id: PlanDecisionId): RemovePlanDecisionResult {
    if (ingress.status === "recoveryRequired") return { status: "notAttempted", reason: "protectedDecisionIngress" };
    if (!decisions.some((decision) => decision.id === id)) return { status: "notAttempted", reason: "notFound" };
  ...
  function removeQuarantinedPlanDecision(quarantineId: string) {
    if (ingress.status === "recoveryRequired") return { status: "notAttempted", reason: "protectedDecisionIngress" } as const;
    if (!quarantined.some((entry) => entry.quarantineId === quarantineId)) return { status: "notAttempted", reason: "notFound" } as const;
  ...
  function exportQuarantinedPlanDecision(quarantineId: string) {
    ...
    return entry ? { status: "exported", entry: cloneQuarantine(entry) } as const
  ...
  function recheck() {
    ...
    try { return storage.getItem(DAYFRAME_PLAN_DECISIONS_STORAGE_KEY) === protectedRaw ? "unchanged" as const : "sourceChanged" as const; }
  ...
  function clear(): PersistenceRemovalOutcome {
    ...
    try {
      ...
      decisions = []; quarantined = []; protectedRaw = undefined;
      setDurability("durable"); setIngress({ status: "noSource", reason: "missing" });
      options.onAuthorityChanged?.(); notifyDecisions();
      return { status: "removed" };
    } catch { setDurability("storageFailure"); return { status: "storageFailure" }; }
  }

  return {
    getPlanDecisions, getQuarantinedPlanDecisions, getPlanDecisionDurabilityStatus,
    getPlanDecisionIngressStatus, acceptPlanDecision, removePlanDecision,
    retryPlanDecisionPersistence, removeQuarantinedPlanDecision, exportQuarantinedPlanDecision,
    exportProtectedPlanDecisionSource, recheckProtectedPlanDecisionSource: recheck,
    replaceProtectedPlanDecisionCheckpoint: () => recover("replace"),
    abandonProtectedPlanDecisionCheckpoint: () => recover("abandon"), clearPlanDecisions: clear,
...
function load(storage: Storage | undefined) {
  if (!storage) return { decisions: [] as PlanDecisionV1[], quarantined: [] as QuarantinedPlanDecision[],
  ...
  let raw: string | null;
  try { raw = storage.getItem(DAYFRAME_PLAN_DECISIONS_STORAGE_KEY); }
  catch { return { decisions: [], quarantined: [], ingress: { status: "recoveryRequired", reason: "readFailure", sourcePreserved: false } as PlanDecisionIngressStatus,
    durability: "storageFailure" as SurfaceDurabilityStatus, protectedRaw: undefined }; }
  if (raw === null) return { decisions: [], quarantined: [], ingress: { status: "noSource", reason: "missing" } as PlanDecisionIngressStatus,
    durability: "unknown" as SurfaceDurabilityStatus, protectedRaw: undefined };
  let parsed: unknown;
  try { parsed = JSON.parse(raw) as unknown; }
  catch { return protectedLoad("corruptJson", raw); }
  if (!record(parsed) || parsed.app !== "DayFrame" || parsed.surface !== "planDecisions" || !Array.isArray(parsed.decisions)) return protectedLoad("invalidEnvelope", raw);
  if (parsed.version !== PLAN_DECISION_SURFACE_VERSION) return protectedLoad("unsupportedVersion", raw);
  const decisions: PlanDecisionV1[] = []; const quarantined: QuarantinedPlanDecision[] = [];
  const ids = new Set<string>(); const targets = new Set<string>();
  parsed.decisions.forEach((entry, index) => {
    ...
    if (reason) quarantined.push({ quarantineId: `plan-decision-quarantine-${index}`, reason, raw: cloneRaw(entry) });
  });
  return { decisions: decisions.sort(compareDecisions), quarantined,
    ingress: { status: "accepted", quarantinedEntryCount: quarantined.length } as PlanDecisionIngressStatus,
...
function protectedLoad(reason: "corruptJson" | "invalidEnvelope" | "unsupportedVersion", raw: string) {
  return { decisions: [] as PlanDecisionV1[], quarantined: [] as QuarantinedPlanDecision[],
    ingress: { status: "recoveryRequired", reason, sourcePreserved: true } as PlanDecisionIngressStatus,
    durability: "unknown" as SurfaceDurabilityStatus, protectedRaw: raw };
}
function envelope(entries: unknown[]): PlanDecisionEnvelopeV1 { return { app: "DayFrame", surface: "planDecisions", version: 1, decisions: entries.map(cloneRaw) }; }
function verifiedEnvelope(raw: string | null, desired: unknown[]): boolean {
  if (raw === null) return false;
  try {
    const value = JSON.parse(raw) as unknown;
    return record(value) && value.app === "DayFrame" && value.surface === "planDecisions" &&
      value.version === PLAN_DECISION_SURFACE_VERSION && Array.isArray(value.decisions) &&
      JSON.stringify(value.decisions) === JSON.stringify(desired);
  } catch { return false; }
}
function cloneEntries(decisions: PlanDecisionV1[], quarantine: QuarantinedPlanDecision[]) { return [...decisions.map(clonePlanDecision), ...quarantine.map((entry) => cloneRaw(entry.raw))]; }
function cloneQuarantine(entry: QuarantinedPlanDecision): QuarantinedPlanDecision { return { ...entry, raw: cloneRaw(entry.raw) }; }
