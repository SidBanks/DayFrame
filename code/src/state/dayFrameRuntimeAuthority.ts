...
export const DAYFRAME_RUNTIME_AUTHORITY_CAPABILITY: unique symbol =
  Symbol("DayFrameRuntimeAuthorityCapability");

export type RuntimeAuthorityAdapter<T = unknown> = {
  id: "active" | "profiles" | "planDecisions" | "executionHistory" | "historicalPlan";
  captureRuntimeSnapshot: () => T;
  installRuntimeExact: (target: T) => void;
};

export type DayFrameRuntimeAuthorityController = {
  begin: (kind?: "bootstrap" | "restore" | "internalReplacement") =>
    { status: "begun"; epoch: number } | { status: "busy" | "snapshotFailed" };
  install: (participantId: RuntimeAuthorityAdapter["id"], target: unknown) =>
...
export function registerDayFrameRuntimeAuthorityController(store: object,
  controller: DayFrameRuntimeAuthorityController): void {
  controllers.set(store, controller);
}

export function getDayFrameRuntimeAuthorityController(store: DayFrameStore,
  capability: typeof DAYFRAME_RUNTIME_AUTHORITY_CAPABILITY): DayFrameRuntimeAuthorityController {
