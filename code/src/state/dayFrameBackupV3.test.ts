...
describe("Backup V3 domain format", () => {
  it("strictly validates, clone-isolates, JSON-roundtrips, and excludes exportedAt from identity", () => {
    ...
    const parsed = validateDayFrameBackupV3(JSON.parse(JSON.stringify(first)));
    expect(parsed).toEqual(first);
    expect(backupV3SemanticFingerprint(first)).toBe(
      backupV3SemanticFingerprint(emptyBackup("2026-08-23T12:00:00.000Z")),
    );
  ...
  it("rejects wrong app, extra keys, noncanonical time, and invalid nested authority", () => {
    ...
    expect(() =>
      validateDayFrameBackupV3({ ...backup, exportedAt: "2026-08-22T07:00:00-05:00" }),
    ).toThrow();
    expect(() =>
      validateDayFrameBackupV3({
        ...backup,
        data: { ...backup.data, historicalPlan: { surfaceVersion: 1, batches: [{ bad: true }] } },
      }),
    ).toThrow();
  });
});
...
describe("Backup V3 five-surface integration", () => {
  it("exports, completely restores through the coordinator, and survives restart", async () => {
    ...
    const name = "backup-v3-roundtrip";
    const setup = () => {
      const db = createDayFrameDurableDb({ indexedDB: factory, name });
      return {
        db,
        store: createDayFrameStore(undefined, {
    ...
    const first = setup();
    expect(await first.store.whenReady()).toEqual({ status: "ready" });
    expect(first.store.setSchedulingPreferences({ dayBoundaryStartTime: "02:00" }).status).toBe(
      "applied",
    );
    ...
    expect(first.store.setSchedulingPreferences({ dayBoundaryStartTime: "03:00" }).status).toBe(
      "applied",
    );
    ...
    expect(await restarted.store.whenReady()).toEqual({ status: "ready" });
