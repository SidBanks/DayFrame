...
describe("durability semantics", () => {
  it.each([
    ...
    "classifies write outcome %s as %s",
    (status, expected) => {
  ...
  it.each([
    ...
    "classifies removal outcome %s as %s",
    (status, expected) => {
  ...
  it.each([
    ...
    "classifies retained status %s as %s",
    (status, expected) => {
  ...
  it.each([
    [
      { activeState: { status: "removed" }, profiles: { status: "removed" }, durability: "cleared" },
    ...
    expect(classifyClearLocalDataResult(result)).toEqual(expected);
  });
  ...
  it("classifies store mutation and two-surface retained status through shared mappings", () => {
    ...
    expect(
      classifyStoreMutationResult({ state, persistence: { status: "storageFailure" } }),
    ).toBe("retryableStorageFailure");
    expect(
      classifyStoreDurabilityStatus({
