...
class MemoryStorage implements Storage {
  ...
  removeItem(key: string) {
    if (this.failWrite) throw new Error("remove");
...
function storeProviders() {
  ...
  return {
    allocateExecutionRecordId: value.allocateRecordId,
    allocateExecutionSubjectId: value.allocateSubjectId,
    executionHistoryClock: value.now,
  };
}
...
describe("ExecutionHistory V1 ingress and envelope", () => {
  it("uses an independent key/version and treats no key as healthy empty history", () => {
    ...
    expect(surface.getExecutionHistoryIngressStatus()).toEqual({
      status: "noSource",
      reason: "missing",
    });
  ...
  it("rehydrates exact records and outcomes without source resolution or allocation", () => {
    ...
    const restarted = createExecutionHistorySurface({
      allocateRecordId: allocate,
      allocateSubjectId: allocate,
    });
  ...
  it.each([
    ...
    expect(surface.recordExecution(plannedInput())).toEqual({
      status: "rejected",
      reason: "protectedHistoryIngress",
    });
    expect(surface.retryExecutionHistoryPersistence()).toEqual({
      status: "notAttempted",
  ...
  it("uses source recheck for protected replace and abandon", () => {
    ...
    expect(changed.replaceProtectedExecutionHistoryCheckpoint()).toEqual({
      status: "notAttempted",
    ...
    expect(replace.replaceProtectedExecutionHistoryCheckpoint()).toMatchObject({
      status: "resolved",
      action: "replace",
    });
    ...
    expect(abandon.abandonProtectedExecutionHistoryCheckpoint()).toMatchObject({
      status: "resolved",
      action: "abandon",
    });
    expect(createExecutionHistorySurface().getExecutionHistoryIngressStatus()).toMatchObject({
      status: "accepted",
    });
  });
});
...
describe("component quarantine", () => {
  it("preserves valid independent subjects and quarantines malformed/unsupported components", () => {
    ...
    storage.setItem(
      DAYFRAME_EXECUTION_HISTORY_STORAGE_KEY,
      JSON.stringify({ ...envelope, records: [valid, malformed, unsupported] }),
    );
    ...
    expect(loaded.getExecutionHistoryIngressStatus()).toEqual({
      status: "accepted",
  ...
  it("quarantines all duplicate-planned and duplicate-ID implicated subjects", () => {
    ...
    storage.setItem(
      DAYFRAME_EXECUTION_HISTORY_STORAGE_KEY,
      JSON.stringify({ ...envelope, records: [first, duplicatePlanned] }),
    );
    ...
    expect(
      plannedConflict
        ...
        .every((entry) => entry.reason === "duplicatePlannedReference"),
    ).toBe(true);

    const second = { ...structuredClone(first), subjectId: subjectIds[1] };
    storage.setItem(
      DAYFRAME_EXECUTION_HISTORY_STORAGE_KEY,
      JSON.stringify({ ...envelope, records: [first, second] }),
    );
    ...
    expect(
      idConflict
        .getQuarantinedExecutionHistory()
        .every((entry) => entry.reason === "duplicateRecordId"),
    ).toBe(true);
  });
  ...
  it("quarantines both sides of cross-subject replacement", () => {
    ...
    const otherRoot = {
      ...
      snapshot: { ...snapshot, sourceFamily: "unplanned", plan: { state: "unplanned" } },
    };
    const cross = {
      ...structuredClone(first),
      id: recordIds[2],
      ...
      subject: otherRoot.subject,
      snapshot: otherRoot.snapshot,
      recordedAt: "2026-08-20T19:00:00.000Z",
    };
    storage.setItem(
      DAYFRAME_EXECUTION_HISTORY_STORAGE_KEY,
      JSON.stringify({ ...envelope, records: [first, otherRoot, cross] }),
    );
    const loaded = createExecutionHistorySurface();
  ...
  it("persists, exports, clone-isolates, and explicitly removes quarantine", () => {
    ...
    storage.setItem(
      DAYFRAME_EXECUTION_HISTORY_STORAGE_KEY,
      JSON.stringify({ ...envelope, records: [...envelope.records, { bad: true }] }),
    );
    ...
    expect(loaded.removeQuarantinedExecutionHistory(id)).toMatchObject({
      status: "removed",
...
describe("store-owned report, correction, retraction, and durability", () => {
  it("appends planned/unplanned reports, prevents duplicate planned subjects and rejects unplanned skip", () => {
    ...
    const unplannedSnapshot = {
      ...snapshot,
      sourceFamily: "unplanned" as const,
      plan: { state: "unplanned" as const },
    };
    expect(
  ...
  it("appends correction/retraction/restoration and rejects stale heads", () => {
    ...
    expect(
      surface.correctExecutionRecord(first.record.subjectId, first.record.id, {
        snapshot,
        outcome: "completed",
      }),
    ).toEqual({ status: "rejected", reason: "notCurrentHead" });
    if (corrected.status !== "accepted") return;
    const retracted = surface.retractExecutionRecord(
      first.record.subjectId,
      corrected.record.id,
      "incorrect",
    );
    expect(surface.getExecutionOutcome(first.record.subjectId)).toEqual({ status: "unknown" });
    if (retracted.status !== "accepted") return;
    const restored = surface.correctExecutionRecord(first.record.subjectId, retracted.record.id, {
      snapshot,
      outcome: "completed",
    });
    expect(restored.status).toBe("accepted");
    expect(surface.getExecutionOutcome(first.record.subjectId)).toMatchObject({
      status: "completed",
    });
  });
  ...
  it("keeps runtime authority after quota failure and retries exact desired checkpoint without allocation", () => {
    ...
    const surface = createExecutionHistorySurface({
      allocateRecordId: allocateRecord,
      allocateSubjectId: allocateSubject,
      now,
    });
  ...
  it("clone-isolates history subscriptions and separates durability-only retry notifications", () => {
    const surface = createExecutionHistorySurface(providers());
    const history = vi.fn((value) => {
      if (value[0]?.kind === "assertion") value[0].snapshot.title = "mutated";
    });
