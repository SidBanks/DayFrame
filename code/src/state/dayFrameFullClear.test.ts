...
describe("ten-authority full-clear settlement", () => {
  it("does not return a terminal result while HistoricalPlan clear is pending", async () => {
    ...
    const base = createDayFrameDurableDb({
      indexedDB: new IDBFactory(),
      name: "delayed-historical-clear",
    });
    let release!: () => void;
  ...
  it("does not return a terminal result while established ExecutionHistory clear is pending", async () => {
    ...
    const db = createDayFrameDurableDb({ indexedDB: factory, name: "delayed-execution-clear" });
    ...
    const indexed = {
      ...baseIndexed,
      clearEstablishedAuthority: async () => {
        await gate;
        return baseIndexed.clearEstablishedAuthority();
      },
    };
  ...
  it("classifies mixed and all-failed terminal outcomes from all ten enumerable authorities", async () => {
    ...
    const failingStorage = createDayFrameDurableDb({
      indexedDB: new IDBFactory(),
      name: "failed-historical-clear",
    });
    const historicalPlanSurface = createHistoricalPlanSurface({
      storage: {
        ...failingStorage,
        mutate: async () => ({
          ...
          error: {
            ...
            operation: "clearHistoricalPlan",
          },
        }),
      },
    });
    const mixed = await createReadyDayFrameTestStore(undefined, {
    ...
    delete (globalThis as { localStorage?: unknown }).localStorage;
    const allFailedSurface = createHistoricalPlanSurface({
      storage: {
        ...failingStorage,
        mutate: async () => ({
          status: "failure" as const,
    ...
    const failed = await createReadyDayFrameTestStore(undefined, {
      historicalPlanSurface: allFailedSurface,
    }).clearLocalData();
    expect(failed.status).toBe("partiallyCleared");
    expect(failed.durability).toBe("partiallyCleared");
  });
});
