...
export type DayFrameMutationAdmission = { allowed: true } | {
  allowed: false;
  reason: "initializing" | "protected" | "authorityTransactionActive";
};

export class DayFrameMutationAdmissionError extends Error {
  constructor(public readonly reason: Exclude<DayFrameMutationAdmission, { allowed: true }>["reason"]) {
    super(`DayFrame mutation is blocked: ${reason}.`);
    this.name = "DayFrameMutationAdmissionError";
  }
}

export function getDayFrameMutationAdmission(readiness: DayFrameReadiness,
  transactionActive: boolean): DayFrameMutationAdmission {
