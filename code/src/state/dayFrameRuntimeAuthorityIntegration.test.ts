...
import { DAYFRAME_RUNTIME_AUTHORITY_CAPABILITY,
  getDayFrameRuntimeAuthorityController } from "./dayFrameRuntimeAuthority.js";
...
describe("five-participant runtime authority integration", () => {
  it("installs all five targets before one coherent notification flush", () => {
    ...
    const unsubscribers = [store.subscribe(crossRead), store.subscribeProfileIngress(crossRead),
      store.subscribePlanDecisions(crossRead), store.subscribeExecutionHistory(crossRead),
    ...
    profiles.profiles = [];
    const decisions = structuredClone(target.planDecisions!); decisions.durability = "pending";
    const execution = structuredClone(target.executionHistory!); execution.durability = "pending";
    const historical = structuredClone(target.historicalPlan!);
    historical.status = { status: "pending", pendingCount: 0 };
    for (const [id, value] of Object.entries({ active, profiles, planDecisions: decisions,
      executionHistory: execution, historicalPlan: historical })) {
    ...
    unsubscribeMutation(); for (const unsubscribe of unsubscribers) unsubscribe();
  });
  ...
  it("restores exact clone-isolated five-participant snapshots on abort without persistence", () => {
    ...
    (target.historicalPlan as Record<string, unknown>).status = { status: "protected",
