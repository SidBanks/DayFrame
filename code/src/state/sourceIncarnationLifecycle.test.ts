...
const pattern: DayFrameAuthoredPattern = {
  ...
  shiftCycles: [{ id: "cycle-1", userId: "user-1", name: "Cycle", type: "fixedSegments",
    mode: "manualSegments", startsOnDate: "2026-05-01", endsOnDate: "2026-05-31",
    sequenceAnchorDate: "2026-05-01", createdAt: "2026-05-01T00:00:00Z",
    updatedAt: "2026-05-01T00:00:00Z",
    segments: [{ id: "segment-1", shiftCycleId: "cycle-1", shiftDefinitionId: "shift-1",
      startsOnDate: "2026-05-01", endsOnDate: "2026-05-31" }],
  ...
  manualEvents: [{ id: "event-1", title: "Event", userDayDate: "2026-05-04", allDay: true,
    createdAt: "2026-05-01T00:00:00Z", updatedAt: "2026-05-01T00:00:00Z" }],
};
...
const setupOperations = (operation: "update" | "replace") => ([
  ...
  { sourceKind: "shiftSequenceEntry", parentSourceId: "cycle-1", sourceId: "sequence-1", operation },
  { sourceKind: "blockTemplate", sourceId: "template-1", operation },
  { sourceKind: "blockRecurrence", sourceId: "recurrence-1", operation },
] satisfies Array<{ sourceKind: AuthoredSourceKind; sourceId: string; parentSourceId?: string;
...
describe("source incarnation lifecycle authority", () => {
  ...
  it("allocates fresh setup lifetimes on replace and preserves nested identity through reorder", () => {
    ...
    const authored = projectActiveToPattern(store.getState());
    const setup = setupInput(authored);
    store.commitAuthoredSetupTransaction({ authoredSetup: setup,
      lifecycle: { operations: setupOperations("replace") } });
    const replaced = identities(store.getState());
    for (const kind of Object.keys(before).filter((key) => key !== "manualEvent") as Array<keyof typeof before>) {
