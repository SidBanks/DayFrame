...
export type AuthorityTransactionState =
  | { status: "inactive" }
...
export function createDayFrameAuthorityTransaction(options: {
  ...
  function commit() {
    ...
    const current = state;
    state = { ...current, status: "flushing" };
    try {
      options.scheduler.commit();
      snapshot = undefined;
      state = { status: "inactive" };
      return { status: "committed" as const };
    } catch {
      state = current;
      return { status: "flushFailed" as const };
    }
  }
  function install(participantId: string, value: unknown) {
    ...
    try {
      participant.installExact(structuredClone(value));
      return { status: "installed" as const };
    } catch {
      return { status: "installFailed" as const };
    }
  }
  function abort() {
    if (state.status !== "active" || !snapshot) return { status: "notActive" as const };
    const current = state;
    state = { ...current, status: "aborting" };
    try {
      for (const participant of options.participants)
        participant.installExact(structuredClone(snapshot.get(participant.id)));
      options.scheduler.abort();
      snapshot = undefined;
      state = { status: "inactive" };
      return { status: "aborted" as const };
    } catch {
      return { status: "abortFailed" as const };
    }
  }
  function getState(): AuthorityTransactionState {
    return { ...state };
  }
  function captureAll() {
    return Object.fromEntries(
      options.participants.map((participant) => [
        participant.id,
        structuredClone(participant.capture()),
      ]),
    );
  }
