import { createIndexedDbCollectionStorage, DAYFRAME_DURABLE_DB_NAME, DAYFRAME_DURABLE_DB_VERSION,
  type IndexedDbCollectionStorage } from "./indexedDbCollectionStorage.js";
...
export function createDayFrameDurableDb(options: { indexedDB?: IDBFactory; name?: string } = {}): IndexedDbCollectionStorage {
  return createIndexedDbCollectionStorage({
    schema: { name: options.name ?? DAYFRAME_DURABLE_DB_NAME, version: DAYFRAME_DURABLE_DB_VERSION, stores: [
      { name: HISTORICAL_PLAN_BATCH_STORE, keyPath: "batchId", indexes: [{ name: HISTORICAL_PLAN_BATCH_TIME_INDEX, keyPath: "publishedAt" }] },
      { name: HISTORICAL_PLAN_DAY_STORE, keyPath: ["batchId", "userDayDate"], indexes: [
      ...
      { name: EXECUTION_HISTORY_RECORD_STORE, keyPath: "recordId", indexes: [
