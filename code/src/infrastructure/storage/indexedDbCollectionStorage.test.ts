...
import {
  ...
  type DurableDatabaseSchema,
} from "./indexedDbCollectionStorage.js";

const schema = (name: string, version = 1): DurableDatabaseSchema => ({
  name,
  version,
  stores: [
    {
      ...
      indexes: [
        ...
        { name: "byDayAndTime", keyPath: ["day", "time"] },
...
describe("IndexedDB durable collection storage", () => {
  it("puts, gets, deletes, clears, and preserves structured-clone isolation", async () => {
    ...
    expect(await storage.get<typeof input>("items", "a")).toMatchObject({
    ...
    await storage.putMany("items", [
      { id: "b", slug: "b", day: "2026-08-20", time: "10:00" },
    ...
    const result = await storage.mutate([
      {
        type: "put",
        store: "items",
        value: { id: "a", slug: "a", day: "2026-08-20", time: "09:00" },
      },
      { type: "put", store: "metadata", value: { key: "checkpoint", value: 1 } },
    ]);
    expect(result.status).toBe("success");
    expect(await storage.get("items", "a")).toMatchObject({
      status: "success",
      value: { id: "a" },
    });
    expect(await storage.get("metadata", "checkpoint")).toMatchObject({
      status: "success",
      value: { value: 1 },
    });
  });
  ...
  it("normalizes quota-like write failure", async () => {
    ...
    expect(await storage.put("items", value)).toMatchObject({
      status: "failure",
  ...
  it("supports equality, bounded compound ranges, and reverse limited queries", async () => {
    ...
    expect(
      await storage.queryIndex<{ id: string }>({
        store: "items",
        index: "byDay",
        query: "2026-08-20",
      }),
    ...
    expect(
      await storage.queryIndex<{ id: string }>({
        store: "items",
        index: "byDayAndTime",
        query: IDBKeyRange.bound(["2026-08-20", "10:00"], ["2026-08-21", "08:00"]),
        direction: "prev",
        limit: 2,
      }),
  ...
  it("upgrades additively, preserves data, and closes stale connections on versionchange", async () => {
    ...
    const first = createIndexedDbCollectionStorage({
      schema: { name, version: 1, stores: [{ name: "items", keyPath: "id" }] },
      indexedDB: factory,
      onVersionChange: () => {
        versionChanged = true;
      },
    ...
    const second = createIndexedDbCollectionStorage({
      schema: {
        name,
        version: 2,
        stores: [
    ...
    expect(
      await second.queryIndex({ store: "items", index: "byDay", query: "2026-08-20" }),
  ...
  it("aborts an invalid upgrade without destroying the prior database", async () => {
    ...
    await first.put("items", { id: "kept" });
    first.close();
    const invalidUpgrade = createIndexedDbCollectionStorage({
      schema: {
        name,
        version: 2,
        stores: [
          {
            name: "items",
            keyPath: "id",
            indexes: [{ name: "invalid", keyPath: ["a", "b"], multiEntry: true }],
          },
        ],
      },
      indexedDB: factory,
    ...
    const reopened = createIndexedDbCollectionStorage({
      schema: { name, version: 1, stores: [{ name: "items", keyPath: "id" }] },
      indexedDB: factory,
    });
    expect(await reopened.get("items", "kept")).toMatchObject({
      status: "success",
      value: { id: "kept" },
    });
  });
  ...
  it("reports a blocked upgrade instead of hanging", async () => {
    ...
    let blocked = false;
    const storage = createIndexedDbCollectionStorage({
      schema: { name, version: 2, stores: [{ name: "items", keyPath: "id" }] },
      indexedDB: factory,
      onBlocked: () => {
        blocked = true;
      },
    });
    expect(await storage.open()).toEqual({
      status: "failure",
      error: { code: "upgradeBlocked", operation: "open" },
    });
  ...
  it("normalizes open failures and remains import/construction safe", async () => {
    ...
    expect(opens).toBe(0);
    expect(await storage.open()).toMatchObject({
      status: "failure",
      error: { code: "openFailed", name: "SecurityError" },
    });
  });
});
