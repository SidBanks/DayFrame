...
const transitions: Record<RestoreStage, readonly RestoreStage[]> = {
  prepared: ["staged"],
  staged: ["indexedDbCommitted", "rollbackPrepared"],
  ...
  verified: ["finalized", "recoveryRequired"],
  finalized: [],
  ...
  rollbackVerified: ["finalized", "recoveryRequired"],
  recoveryRequired: [],
};

export function validateRestoreJournal(
  value: unknown,
): { status: "valid"; journal: RestoreJournalV1 } | { status: "invalid" } {
  if (
    !record(value) ||
    !exact(value, [
      "app",
  ...
  if (
    value.app !== "DayFrame" ||
    value.surface !== "restore-journal" ||
    value.version !== RESTORE_JOURNAL_VERSION ||
    !isRestoreTransactionId(value.transactionId) ||
    !RESTORE_STAGES.includes(value.stage as RestoreStage) ||
    (value.mode !== "ordinary" && value.mode !== "recoveryReplacement") ||
    !fingerprint(value.targetFingerprint) ||
    !fingerprint(value.recoveryFingerprint) ||
...
export function createRestoreJournalStorage(
  storage: Pick<Storage, "getItem" | "setItem" | "removeItem">,
) {
  function read() {
    let raw: string | null;
    try {
      raw = storage.getItem(RESTORE_JOURNAL_KEY);
    } catch {
      return { status: "failure" as const, reason: "storageFailure" as const };
    }
    if (raw === null) return { status: "absent" as const };
    try {
      const checked = validateRestoreJournal(JSON.parse(raw));
      return checked.status === "valid"
        ? { status: "present" as const, journal: checked.journal }
        : { status: "failure" as const, reason: "invalidJournal" as const };
    } catch {
      return { status: "failure" as const, reason: "invalidJournal" as const };
    }
  }
  function write(journal: RestoreJournalV1) {
    if (validateRestoreJournal(journal).status !== "valid")
      return { status: "failure" as const, reason: "invalidJournal" as const };
    const current = read();
    if (current.status === "failure") return current;
    if (
      current.status === "present" &&
      (current.journal.transactionId !== journal.transactionId ||
        (current.journal.stage !== journal.stage &&
          !canTransitionRestoreJournal(current.journal.stage, journal.stage)))
    )
    ...
    return verified.status === "present" &&
      JSON.stringify(verified.journal) === JSON.stringify(journal)
      ? { status: "success" as const }
      : { status: "failure" as const, reason: "verificationFailed" as const };
  }
  function clear(transactionId: RestoreTransactionId) {
    const current = read();
    if (current.status !== "present" || current.journal.transactionId !== transactionId)
