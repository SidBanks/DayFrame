...
} from "../core/execution/historicalExecutionTarget.js";
import type {
...
export function ExecutionReportControl(props: ExecutionReportControlProps): ReactElement {
  ...
  const [draft, setDraft] = useState<ExecutionReportDraft>({
    outcome: "completed",
    occurredAtLocal: "",
    durationMinutes: "",
    note: "",
  ...
  const materialized =
    "target" in props
      ? { status: "materialized" as const, target: structuredClone(props.target) }
      : materializeHistoricalExecutionTarget({
          authoredSetup: props.authoredSetup,
          preview: props.preview,
          selection: props.selection,
        });
  const target = materialized.status === "materialized" ? materialized.target : null;
  const subjectId = target
    ? store.findExecutionSubjectByPlannedReference(target.reference)
    : undefined;
  const outcome = subjectId ? store.getExecutionOutcome(subjectId) : { status: "unknown" as const };
  const current =
    outcome.status === "completed" || outcome.status === "partial" || outcome.status === "skipped"
      ? outcome.record
      : undefined;
  const currentHead = subjectId
    ? [...records].reverse().find((record) => record.subjectId === subjectId)
    : undefined;
  ...
  function submit(event: FormEvent): void {
    ...
    if (!retainedTarget || !target || !executionTargetsEqual(retainedTarget, target)) {
      setMessage("The planned context changed. Review it before reporting.");
      setOpen(false);
      setRetainedTarget(null);
      return;
    }
    ...
    const existingOutcome = existing
      ? store.getExecutionOutcome(existing)
      : { status: "unknown" as const };
    const historicalAssertion = existing
      ? [...records]
          .reverse()
          .find(
            (record): record is ExecutionAssertionRecordV1 =>
              record.subjectId === existing && record.kind === "assertion",
          )
      : undefined;
    const result =
      existing && existingOutcome.status === "unknown" && currentHead
        ? store.correctExecutionRecord(existing, currentHead.id, {
            snapshot: historicalAssertion?.snapshot ?? built.input.snapshot,
            subject: historicalAssertion?.subject ?? built.input.subject,
    ...
    if (result.status === "rejected") {
      setMessage(
        result.reason === "duplicatePlannedSubject"
          ? "This occurrence already has a report."
    ...
    setMessage(
      result.persistence.status === "persisted"
        ? "Report recorded."
        : "Recorded for this session; saving failed.",
    );
  }
  ...
  function undo(): void {
    ...
    setMessage(
      result.status === "accepted"
        ? result.persistence.status === "persisted"
          ? "Report undone."
          : "Undone for this session; saving failed."
        : "The report could not be undone.",
    );
  }
  ...
  if (ingress.status === "recoveryRequired")
    return (
      <span className="df-danger-message">
        {" "}
        Reporting is unavailable until execution-history recovery is resolved.
      </span>
    );
  return (
    <div className="df-execution-report">
      {current ? (
        <div role="status">
          <strong>{label(current)}</strong> · {context(current)}
          <button className="df-secondary-button" onClick={undo} type="button">
            Undo report
          </button>
        </div>
      ) : (
        <button
          ...
          onClick={() => {
            setOpen(true);
            setRetainedTarget(structuredClone(target));
            setMessage("");
          }}
          type="button"
        >
      ...
      {open ? (
        <form className="df-form-stack" onSubmit={submit}>
          <p>
            <strong>{target.snapshot.title}</strong> · {snapshotContext(target)}
          </p>
          <fieldset className="df-fieldset">
            <legend>Outcome</legend>
            {(["completed", "partial", "skipped"] as const).map((value) => (
              <label key={value}>
                <input
                  checked={draft.outcome === value}
                  name={`outcome-${contextKey}`}
                  onChange={() =>
                    setDraft({
                      ...draft,
                      outcome: value,
                      ...(value === "skipped" ? { occurredAtLocal: "", durationMinutes: "" } : {}),
                    })
                  }
                  type="radio"
                />{" "}
                {value === "completed" ? "Complete" : value === "partial" ? "Partial" : "Skip"}
              </label>
            ))}
          </fieldset>
          ...
          <div>
            ...
            <button className="df-secondary-button" onClick={() => setOpen(false)} type="button">
              Cancel
            </button>
          </div>
        </form>
      ...
      {saveFailed ? (
        <div className="df-danger-message" role="status">
          <p>Execution history saving failed; the session outcome is still active.</p>
...
function label(record: ExecutionAssertionRecordV1): string {
...
function context(record: ExecutionAssertionRecordV1): string {
  return `${record.snapshot.title}, ${record.snapshot.userDay.date}, ${record.snapshot.plan.state}`;
}
function snapshotContext(target: HistoricalExecutionTarget): string {
  const plan = target.snapshot.plan;
  return plan.state === "scheduled"
    ? `${target.snapshot.userDay.date}, ${formatHumanTimeRange(new Date(plan.startsAt), new Date(plan.endsAt))}`
    : `${target.snapshot.userDay.date}, ${plan.state}`;
}
