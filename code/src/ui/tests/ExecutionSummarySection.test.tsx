...
import type { ExecutionRecordId, ExecutionSubjectId } from "../../core/execution/executionRecord.js";
...
describe("ExecutionSummarySection", () => {
  it("renders categorical all-history counts and explicit no-Preview coverage without percentages", () => {
    const store = createReadyDayFrameTestStore(undefined, {
      ...
      allocateExecutionSubjectId: () => "00000000-0000-4000-8000-000000000002" as ExecutionSubjectId,
      executionHistoryClock: () => "2026-08-21T12:00:00.000Z",
    });
    store.recordExecution({ subject: { kind: "unplanned" }, outcome: "partial", snapshot: {
      sourceFamily: "unplanned", title: "Extra activity", category: "optional",
      userDay: { date: "2026-08-20", dayBoundaryStartTime: "03:00", utcOffsetMinutes: -300 },
      plan: { state: "unplanned" },
    } });
    const { container } = render(<ExecutionSummarySection authoredSetup={store.getState()} preview={null} store={store} />);
    ...
    expect(screen.getByText("Generate a preview to see current reporting coverage.")).toBeInTheDocument();
  ...
  it("suppresses safe-empty counts when history ingress is protected", () => {
    ...
    render(<ExecutionSummarySection authoredSetup={store.getState()} preview={null} store={store} />);
    expect(screen.getByRole("status")).toHaveTextContent("Execution history needs recovery before outcomes can be summarized.");
