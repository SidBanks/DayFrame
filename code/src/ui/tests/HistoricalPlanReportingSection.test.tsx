...
const reference = { version: 1, sourceKind: "work", cycle: { id: "cycle", incarnationId: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa" },
...
const publishedDay: HistoricalPlanDayPublicationV1 = { version: 1, userDayDate: "2026-08-20",
  dayBoundaryStartTime: "03:00", weekStartsOn: "monday", utcOffsetMinutes: -300,
  occurrences: [{ version: 1, reference, sourceFamily: "work", title: "Archived shift", category: "work",
...
describe("HistoricalPlanReportingSection", () => {
  it("distinguishes missing coverage from an explicitly published empty day", async () => {
    ...
    const getHistoricalPlanDay = vi.fn().mockResolvedValueOnce({ status: "unavailableNoPublication", userDayDate: "2026-08-20" })
      .mockResolvedValueOnce({ status: "available", day: { ...publishedDay, userDayDate: "2026-08-21", occurrences: [] },
        batchId: "11111111-1111-4111-8111-111111111111", publishedAt: "2026-08-19T12:00:00.000Z", durability: "durable" });
  ...
  it("submits a frozen historical target through the existing report control", async () => {
    ...
    const getHistoricalPlanDay = vi.fn().mockResolvedValue({ status: "available", day: publishedDay,
      batchId: "11111111-1111-4111-8111-111111111111", publishedAt: "2026-08-19T12:00:00.000Z", durability: "durable" });
    ...
    expect(deriveOutcomeSummary({ records: base.getExecutionHistory() })).toMatchObject({ status: "available",
      summary: { completed: 1, totalSubjects: 1 } });
    const emptyCurrentPreview = { result: { generatedWorkBlocks: [], blockCandidates: [], scheduledBlocks: [],
      unplacedCandidates: [], frictionPoints: [], planDecisionResults: [] }, rangeStartDate: "2026-08-22",
      rangeEndDate: "2026-08-22", planningWindowStart: new Date("2026-08-22T03:00:00.000Z"),
      planningWindowEnd: new Date("2026-08-23T03:00:00.000Z"), generatedAt: "2026-08-22T12:00:00.000Z",
      isStale: false } as DayFramePreview;
    expect(deriveCurrentPreviewReportingCoverage({ authoredSetup: base.getState(), preview: emptyCurrentPreview,
      records: base.getExecutionHistory() })).toEqual({ status: "available", coverage: {
        eligibleOccurrences: 0, reportedOccurrences: 0, unreportedOccurrences: 0 } });
  });

  it("refreshes a loaded historical day after full clear instead of retaining stale rows", async () => {
    const base = createReadyDayFrameTestStore();
    const getHistoricalPlanDay = vi.fn().mockResolvedValueOnce({ status: "available", day: publishedDay,
      batchId: "11111111-1111-4111-8111-111111111111", publishedAt: "2026-08-19T12:00:00.000Z", durability: "durable" })
      .mockResolvedValue({ status: "unavailableNoPublication", userDayDate: "2026-08-20" });
    render(<HistoricalPlanReportingSection initialDate="2026-08-20" store={{ ...base, getHistoricalPlanDay }}
      now={() => new Date("2026-08-22T12:00:00.000Z")} />);
    expect(await screen.findByText("Archived shift")).toBeInTheDocument();
    await act(async () => { await base.clearLocalData(); });
    expect(await screen.findByText("No published plan history is available for this day.")).toBeInTheDocument();
