...
import { HISTORICAL_METRIC_POLICY_V1, projectHistoricalCompletionDistributionV1,
  type HistoricalCompletionDistributionQueryV1 } from
  "../../core/historicalIntelligence/completionDistribution.js";
import { projectHistoricalSchedulingRealizationV1 } from
...
import type { HistoricalCompletionDistributionQueryResultV1 } from
  "../../state/historicalIntelligenceQuery.js";
import type { HistoricalSchedulingRealizationQueryResultV1 } from
  "../../state/historicalIntelligenceQuery.js";
...
function occurrence(title: string, state: "scheduled" | "unplaced" | "omitted" | "blocked" = "scheduled") {
  return { reference: { version: 1, sourceKind: "manualEvent", manualEvent: {
    id: title, incarnationId: "00000000-0000-4000-8000-000000000001" } },
    sourceFamily: "manualEvent", title, category: "personal",
    plan: state === "scheduled" ? { state, startsAt: "2026-08-20T14:00:00.000Z",
      endsAt: "2026-08-20T15:00:00.000Z" } : { state } } as const;
}
...
function projected(options: { missing?: boolean; empty?: boolean; excluded?: boolean } = {}) {
  const query: HistoricalCompletionDistributionQueryV1 = { policy: HISTORICAL_METRIC_POLICY_V1,
    startUserDayDate: "2026-08-16", endUserDayDate: "2026-08-22",
    evaluationAsOf: "2026-08-22T17:00:00.000Z" };
  const occurrences = options.empty ? [] : [occurrence("Reported shift"),
    ...(options.excluded ? [occurrence("Unplaced block", "unplaced")] : [])];
  const day = { version: 1, userDayDate: "2026-08-20", dayBoundaryStartTime: "03:00",
    weekStartsOn: "monday", utcOffsetMinutes: -300, occurrences } as unknown as HistoricalPlanDayPublicationV1;
  return projectHistoricalCompletionDistributionV1({ query,
    days: [{ day, batchId: "batch-1", publishedAt: "2026-08-20T16:00:00.000Z" }],
    missingUserDayDates: options.missing ? ["2026-08-21"] : [], executionRecords: [] });
}
...
function fakeStore(results: HistoricalCompletionDistributionQueryResultV1[],
  ...
  type ExecutionListener = Parameters<HistoricalIntelligenceSummaryStore["subscribeExecutionHistory"]>[0];
  const history = new Set<HistoryListener>(); const execution = new Set<ExecutionListener>();
  ...
  const getRealization = vi.fn(async (query: HistoricalCompletionDistributionQueryV1) => {
    void query; return realizationResults.shift() ?? realized();
  });
  return { store: { getHistoricalCompletionDistribution: get,
      getHistoricalSchedulingRealization: getRealization,
      subscribeHistory: (listener: HistoryListener) => { history.add(listener); return () => history.delete(listener); },
      subscribeExecutionHistory: (listener: ExecutionListener) => { execution.add(listener); return () => execution.delete(listener); },
    } as HistoricalIntelligenceSummaryStore, get, getRealization };
}
...
describe("HistoricalIntelligenceSummary", () => {
  it("renders the canonical four-state planning projection beside unchanged execution outcomes", async () => {
    const query: HistoricalCompletionDistributionQueryV1 = { policy: HISTORICAL_METRIC_POLICY_V1,
      ...
      evaluationAsOf: "2026-08-22T17:00:00.000Z" };
    const day = { version: 1, userDayDate: "2026-08-20", dayBoundaryStartTime: "03:00",
      weekStartsOn: "monday", utcOffsetMinutes: -300, occurrences: [occurrence("Placed item"),
        occurrence("Loose item", "unplaced"), occurrence("Frozen omission", "omitted"),
        occurrence("Frozen block", "blocked")] } as unknown as HistoricalPlanDayPublicationV1;
    const realization = projectHistoricalSchedulingRealizationV1({ query, days: [{ day,
      batchId: "batch-all", publishedAt: "2026-08-20T16:00:00.000Z" }], missingUserDayDates: [] });
    const controlled = fakeStore([projected({ excluded: true })], [realization]);
    const { container } = render(<HistoricalIntelligenceSummary
      now={() => new Date("2026-08-22T17:00:00.000Z")} store={controlled.store} />);
    expect(await screen.findByRole("heading", { name: "Scheduling realization" })).toBeInTheDocument();
  ...
  it("uses a deliberate seven-day range and explicit evaluation cutoff", async () => {
    ...
    render(<HistoricalIntelligenceSummary now={() => new Date("2026-08-22T17:00:00.000Z")}
    ...
    expect(controlled.get.mock.calls[0]?.[0]).toMatchObject({ startUserDayDate: "2026-08-16",
      endUserDayDate: "2026-08-22", evaluationAsOf: "2026-08-22T17:00:00.000Z" });
  });
  ...
  it("shows all five categorical counts and drill-down provenance without a score", async () => {
    ...
    const { container } = render(<HistoricalIntelligenceSummary now={() => new Date("2026-08-22T17:00:00.000Z")}
      store={controlled.store} />);
  ...
  it("distinguishes incomplete coverage, published-empty history, and zero eligibility", async () => {
    const controlled = fakeStore([projected({ missing: true, empty: true })],
      [realized({ missing: true, empty: true })]);
    render(<HistoricalIntelligenceSummary now={() => new Date("2026-08-22T17:00:00.000Z")}
      store={controlled.store} />);
    expect(await screen.findByText(/Counts below describe known plan history only/)).toBeInTheDocument();
    expect(screen.getByText(/covered day has no planned occurrences/)).toBeInTheDocument();
    expect(screen.getByText(/no scheduled occurrences available for completion analysis/)).toBeInTheDocument();
  ...
  it.each(["historicalPlanProtected", "executionHistoryQuarantined"] as const)(
    "suppresses counts for %s", async (reason) => {
      const controlled = fakeStore([{ status: "unavailable", reason }],
      ...
      expect(await screen.findByText(/protected until|execution evidence cannot be safely included/)).toBeInTheDocument();
  ...
  it("refreshes on both authorities and clears stale projected content while loading", async () => {
    ...
    type ExecutionListener = Parameters<HistoricalIntelligenceSummaryStore["subscribeExecutionHistory"]>[0];
    const history = new Set<HistoryListener>(); const execution = new Set<ExecutionListener>();
    const get = vi.fn().mockResolvedValueOnce(projected()).mockImplementationOnce(() =>
      new Promise<HistoricalCompletionDistributionQueryResultV1>((resolve) => { resolveNext = resolve; }))
      .mockResolvedValueOnce({ status: "unavailable", reason: "historicalPlanUnavailable" });
    const store = { getHistoricalCompletionDistribution: get,
      getHistoricalSchedulingRealization: vi.fn().mockResolvedValue(realized()),
      subscribeHistory: (listener: HistoryListener) => { history.add(listener); return () => history.delete(listener); },
      subscribeExecutionHistory: (listener: ExecutionListener) => { execution.add(listener); return () => execution.delete(listener); } };
    render(<HistoricalIntelligenceSummary now={() => new Date("2026-08-22T17:00:00.000Z")} store={store} />);
    await screen.findByRole("button", { name: "Not reported: 1" });
    ...
    await waitFor(() => expect(screen.getByText(/execution evidence cannot be safely included/)).toBeInTheDocument());
    resolveNext(projected());
    await waitFor(() => expect(screen.queryByRole("button", { name: "Not reported: 1" })).not.toBeInTheDocument());
  });
