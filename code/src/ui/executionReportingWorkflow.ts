...
function buildInput(
  ...
  if (note.length > 2000)
    return { status: "invalid", message: "Note must be 2,000 characters or fewer." };
  ...
  if (draft.occurredAtLocal) {
    ...
    if (Number.isNaN(parsed.getTime()))
      return { status: "invalid", message: "Enter a valid actual date and time." };
  ...
  if (draft.durationMinutes) {
    ...
    if (!Number.isInteger(durationMinutes) || durationMinutes < 1 || durationMinutes > 1440) {
      return {
        status: "invalid",
        message: "Duration must be a whole number from 1 to 1,440 minutes.",
      };
    }
  }
  const actualTime =
...
export function executionTargetsEqual(
  left: HistoricalExecutionTarget,
  right: HistoricalExecutionTarget,
): boolean {
