...
describe("buildAcceptedDecisionViewModels", () => {
  it("presents all four semantic decision kinds with occurrence context", () => {
    const decisions = [
      decision("placeOccurrence"),
      decision("omitOccurrence"),
      decision("setOccurrenceDuration"),
      decision("setOccurrencePriority"),
    ];
    const models = buildAcceptedDecisionViewModels({
      decisions,
      preview: null,
      authoredSetup: state(),
    });
  ...
  it.each([
    ["applied", "Applied"],
    ["blocked", "Blocked"],
    ["outsideWindow", "Outside this preview"],
    ["staleSourceMissing", "Stale"],
    ["staleLifetime", "Stale"],
    ["staleOccurrenceMissing", "Stale"],
    ["inapplicable", "Not currently applicable"],
    ...
    expect(
      buildAcceptedDecisionViewModels({ decisions: [current], preview, authoredSetup: state() })[0],
  ...
  it("does not expose old replay truth from a stale Preview and handles missing fresh results", () => {
    const current = decision("omitOccurrence");
    const stale = freshPreview(current, "applied");
    stale.isStale = true;
    expect(
      buildAcceptedDecisionViewModels({
        decisions: [current],
        preview: stale,
        authoredSetup: state(),
      })[0]?.status,
    ).toBe("regenerateToEvaluate");
    const fresh = freshPreview(current, "applied");
    fresh.result.planDecisionResults = [];
    expect(
      buildAcceptedDecisionViewModels({
        decisions: [current],
        preview: fresh,
  ...
  it("uses a non-retargeting fallback when the source lifetime is unavailable", () => {
    ...
    expect(
      buildAcceptedDecisionViewModels({
        decisions: [current],
        preview: null,
        authoredSetup: { ...state(), blockTemplates: [] },
      })[0]?.summary,
    ).toBe("Omit previous template occurrence");
  });
});
...
function decision(kind: PlanDecisionV1["kind"]): PlanDecisionV1 {
  const base = {
    version: 1 as const,
    id: {
      placeOccurrence: "00000000-0000-4000-8000-000000000001",
      omitOccurrence: "00000000-0000-4000-8000-000000000002",
      setOccurrenceDuration: "00000000-0000-4000-8000-000000000003",
      setOccurrencePriority: "00000000-0000-4000-8000-000000000004",
    }[kind],
    ...
    target: {
      ...
      coordinate: {
        ...
        userDayDate: "2026-05-05" as const,
        slot: 0,
      },
    },
  ...
  if (kind === "placeOccurrence")
    return {
      ...base,
      kind,
      payload: { userDayDate: "2026-05-05", startTime: "09:00" },
    } as unknown as PlanDecisionV1;
  if (kind === "omitOccurrence") return { ...base, kind, payload: {} } as unknown as PlanDecisionV1;
  if (kind === "setOccurrenceDuration")
    return { ...base, kind, payload: { durationMinutes: 30 } } as unknown as PlanDecisionV1;
  return { ...base, kind, payload: { priority: 5 } } as unknown as PlanDecisionV1;
}
function state(): DayFrameState {
  return {
    blockTemplates: [
      { id: "template", incarnationId: "00000000-0000-4000-8000-000000000011", title: "Workout" },
    ],
    manualEvents: [],
    shiftDefinitions: [],
  } as unknown as DayFrameState;
}
function freshPreview(current: PlanDecisionV1, status: string): DayFramePreview {
  return {
    result: {
      generatedWorkBlocks: [],
      blockCandidates: [],
      ...
      planDecisionResults: [
        {
          ...
          ...(status === "inapplicable" ? { reason: "unsupportedTargetFamily" } : {}),
        } as never,
      ],
    },
    rangeStartDate: "2026-05-05",
    rangeEndDate: "2026-05-06",
    planningWindowStart: new Date(2026, 4, 5),
    planningWindowEnd: new Date(2026, 4, 7),
