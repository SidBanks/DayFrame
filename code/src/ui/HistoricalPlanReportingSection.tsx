...
export type HistoricalPlanReportingStore = ExecutionReportingStore &
  Partial<Pick<DayFrameStore, "getHistoricalPlanDay" | "getStatus" | "subscribeHistory">>;

const systemNow = () => new Date();

export function HistoricalPlanReportingSection({
  initialDate,
  store,
  now = systemNow,
}: {
  initialDate: string;
  store: HistoricalPlanReportingStore;
  now?: () => Date;
}): ReactElement {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setResult(null);
    void query(date, now().toISOString()).then(
      (value) => {
      ...
      () => {
        if (request.current === current) {
          setResult({
            status: "unavailable",
            error: { code: "readFailed", operation: "queryHistoricalPlanForReporting" },
          });
          setLoading(false);
        }
      },
  ...
  return (
    <section aria-labelledby="historical-plan-reporting-heading" className="df-summary-bar">
      <h2 className="df-panel-title" id="historical-plan-reporting-heading">
        Report a past planned occurrence
      </h2>
      <p className="df-muted">
        Record what happened to something DayFrame previously had in a planned schedule.
      </p>
      <label className="df-field">
        Past planned date
        <input
          aria-label="Past planned date"
      ...
      {!loading && result?.status === "unavailableNoPublication" ? (
        <p className="df-empty">No published plan history is available for this day.</p>
      ) : null}
      {!loading && result?.status === "available" && result.day.occurrences.length === 0 ? (
        <p className="df-empty">No occurrences were published for this day.</p>
      ) : null}
      {!loading &&
      (result?.status === "unavailableProtected" || result?.status === "unavailable") ? (
        <p className="df-danger-message" role="status">
          Published plan history is unavailable until its stored authority can be read safely.
        </p>
      ) : null}
      {!loading && result?.status === "available" && result.day.occurrences.length > 0 ? (
        <ul className="df-plain-list df-history-list">
          {result.day.occurrences.map((occurrence) => {
            const materialized = materializeHistoricalPlanExecutionTarget({
              day: result.day,
              occurrence,
            });
...
function planLabel(
  plan: import("../core/historicalPlan/historicalPlan.js").HistoricalPlanContext,
): string {
