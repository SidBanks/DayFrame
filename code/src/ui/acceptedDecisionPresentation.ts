...
export function buildAcceptedDecisionViewModels(input: {
  ...
  const replayById =
    input.preview && !input.preview.isStale
      ? new Map(
          input.preview.result.planDecisionResults.map((result) => [result.decisionId, result]),
        )
...
function targetLabel(
  ...
  if (target.sourceKind === "template") {
    const source = state.blockTemplates.find(
      (item) =>
        item.id === target.template.id && item.incarnationId === target.template.incarnationId,
    );
    return source?.title ?? "previous template occurrence";
  }
  if (target.sourceKind === "manualEvent") {
    const source = state.manualEvents.find(
      (item) =>
        item.id === target.manualEvent.id &&
...
function semanticSummary(decision: PlanDecisionV1, target: string): string {
  switch (decision.kind) {
    ...
    case "omitOccurrence":
      return `Omit ${target}`;
    case "setOccurrenceDuration":
      return `Use ${decision.payload.durationMinutes} minutes for ${target}`;
    case "setOccurrencePriority":
      return `Use priority ${decision.payload.priority} for ${target}`;
  }
}
...
function statusCopy(status: AcceptedDecisionStatus): string {
  switch (status) {
    case "applied":
      return "Applied";
    case "blocked":
      return "Blocked — the accepted choice could not be applied in this schedule";
    case "outsideWindow":
      return "Outside this preview";
    case "staleSourceMissing":
      return "Stale — the original source is no longer present";
    case "staleLifetime":
      return "Stale — this choice no longer matches the current source lifetime";
    case "staleOccurrenceMissing":
      return "Stale — this occurrence is not currently generated";
    case "inapplicable":
      return "Not currently applicable";
    case "notEvaluated":
      return "Generate a preview to evaluate this choice";
    case "regenerateToEvaluate":
      return "Regenerate to evaluate this choice";
