...
const range = {
  rangeStartDate: "2026-08-17" as const,
  rangeEndDate: "2026-08-18" as const,
  planningWindowStart: new Date("2026-08-17T03:00:00"),
  planningWindowEnd: new Date("2026-08-19T03:00:00"),
...
function store(seed = pattern) {
  ...
  return createReadyDayFrameTestStore(seed, {
    allocatePlanDecisionId: () =>
      `10000000-0000-4000-8000-${String(next++).padStart(12, "0")}` as PlanDecisionId,
...
function targetForTemplate(current: ReturnType<typeof store>) {
  ...
  const identity = preview.preview!.result.blockCandidates.find(
    (candidate) => candidate.occurrenceIdentity?.sourceKind === "template",
  )!.occurrenceIdentity!;
...
describe("PlanDecision deterministic replay", () => {
  it.each([
    [
      ...
      (result: ReturnType<ReturnType<typeof store>["getState"]>) =>
        expect(
          result.preview!.result.scheduledBlocks.some(
            (block) => block.templateId === "template" && block.userDayDate === "2026-08-17",
          ),
        ).toBe(false),
    ],
    [
      ...
      { durationMinutes: 45 },
      (result: ReturnType<ReturnType<typeof store>["getState"]>) => {
        const block = result.preview!.result.scheduledBlocks.find(
          (candidate) => candidate.templateId === "template",
        )!;
    ...
    [
      ...
      (result: ReturnType<ReturnType<typeof store>["getState"]>) =>
        expect(
          result.preview!.result.scheduledBlocks.find((block) => block.templateId === "template")!
            .priority,
        ).toBe(1),
    ],
    [
      "placeOccurrence",
      { userDayDate: "2026-08-17", startTime: "10:00" },
      (result: ReturnType<ReturnType<typeof store>["getState"]>) =>
        expect(
          result
            .preview!.result.scheduledBlocks.find((block) => block.templateId === "template")!
            .startsAt.getHours(),
        ).toBe(10),
    ],
    ...
    const accepted = current.acceptPlanDecision({
      kind,
      target,
      payload,
      provenance: { source: "user" },
    } as Parameters<typeof current.acceptPlanDecision>[0]);
  ...
  it("blocks an exact placement against an anchored manual event without fallback", () => {
    ...
    current.acceptPlanDecision({
      ...
      target,
      payload: { userDayDate: "2026-08-17", startTime: "10:00" },
      provenance: { source: "user" },
    });
    const result = current.generatePreview(range).preview!.result;
    expect(result.planDecisionResults[0]).toMatchObject({
      status: "blocked",
      reason: "exactPlacementUnavailable",
    });
    expect(result.unplacedCandidates.some((candidate) => candidate.templateId === "template")).toBe(
      true,
    );
  });
  ...
  it("distinguishes missing, mismatched, and occurrence-removed stale authority", () => {
    ...
    expect(
      evaluatePlanDecisionApplicability(accepted.decision, {
        ...current.getState(),
        blockTemplates: [],
      }),
    ).toEqual({ status: "staleSourceMissing" });
    expect(evaluatePlanDecisionApplicability(accepted.decision, store().getState())).toEqual({
      status: "staleLifetime",
    });
    expect(
      evaluatePlanDecisionApplicability(accepted.decision, {
        ...current.getState(),
        blockRecurrences: current.getState().blockRecurrences.map((source) => ({
          ...source,
          startsOnDate: "2026-08-18" as const,
        })),
      }),
