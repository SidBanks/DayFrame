...
import { getPlanDecisionTargetKey, validatePlanDecision,
  type PlanDecisionV1 } from "./planDecision.js";
import { createDurableOccurrenceReference, resolveDurableOccurrenceReference,
...
export function evaluatePlanDecisionApplicability(
  ...
  if (validation.status === "unsupportedVersion" || validation.status === "unsupportedTargetVersion") {
  ...
  const template = authoredSetup.blockTemplates.find(
    (source) => source.id === target.template.id,
  );
...
export function replayPlanDecisions(input: {
  ...
  for (const decision of [...input.decisions].sort(compareDecision)) {
    const base = { decisionId: decision.id, kind: decision.kind,
    ...
    if (decision.kind === "omitOccurrence") {
      ...
      const startsAt = placementDate(decision.payload.userDayDate, decision.payload.startTime,
        input.dayBoundaryStartTime);
      const endsAt = new Date(startsAt.getTime() + candidate.durationMinutes * 60_000);
      if (startsAt < input.planningWindowStart || startsAt >= input.planningWindowEnd ||
          endsAt > input.planningWindowEnd) {
        pendingResults.push({ ...base, status: "outsideWindow" });
        candidates = candidates.filter((current) => current.id !== candidate.id);
        continue;
      }
      candidates[index] = { ...candidate, userDayDate: decision.payload.userDayDate,
        ...
        placementType: "fixed", fixedStartTime: decision.payload.startTime };
      hardPlacementCandidateIds.add(candidate.id);
      exactDecisionCandidateIds.set(candidate.id, clonePlanDecisionForReplay(decision));
    }
  }
  return { blockCandidates: candidates, hardPlacementCandidateIds, pendingResults,
...
export function finalizePlanDecisionResults(
  ...
  const exactResults = [...exact].map(([candidateId, decision]) => {
    const base = { decisionId: decision.id, kind: decision.kind,
      target: structuredClone(decision.target) };
    return { ...base, status: scheduledIds.has(`scheduled_${candidateId}`) ? "applied" : "blocked",
      ...(scheduledIds.has(`scheduled_${candidateId}`) ? {} : { reason: "exactPlacementUnavailable" }) } as PlanDecisionReplayResult;
  });
  const exactIds = new Set([...exact.values()].map((decision) => decision.id));
  return [...pending.filter((result) => !exactIds.has(result.decisionId)), ...exactResults]
    .sort((left, right) => getPlanDecisionTargetKey(left.target).localeCompare(
      getPlanDecisionTargetKey(right.target)) || left.decisionId.localeCompare(right.decisionId));
}
...
function cloneCandidate(candidate: BlockCandidate): BlockCandidate {
  return { ...candidate, externalResources: candidate.externalResources.map((resource) => ({ ...resource })) };
}
...
function compareDecision(left: PlanDecisionV1, right: PlanDecisionV1): number {
  return getPlanDecisionTargetKey(left.target).localeCompare(getPlanDecisionTargetKey(right.target)) ||
