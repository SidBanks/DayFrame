...
export function classifySuggestedFixes(input: {
  ...
  const constraining = [...input.decisions]
    .filter((decision) => {
    ...
    .sort(
      (left, right) =>
        getPlanDecisionTargetKey(left.target).localeCompare(
          getPlanDecisionTargetKey(right.target),
  ...
  return input.frictionPoints.map((friction) => {
    const classified = friction.suggestedFixes
      .map((fix, index) =>
        classifyFix({
          ...input,
          friction,
    ...
    classified.sort(
      (left, right) =>
        rank(left.context.relationship, blockedFriction) -
          rank(right.context.relationship, blockedFriction) ||
        left.originalIndex - right.originalIndex,
    );
    return {
      ...friction,
      affectedBlockIds: [...friction.affectedBlockIds],
...
function classifyFix(
  input: Parameters<typeof classifySuggestedFixes>[0] & {
  ...
  if (prospective?.status === "supported") {
    ...
    const current = input.constraining.find(
      (decision) => getPlanDecisionTargetKey(decision.target) === key,
    );
    if (current) {
      const equivalent =
        current.kind === prospective.candidate.kind &&
        JSON.stringify(current.payload) === JSON.stringify(prospective.candidate.payload);
      return result(
        input,
        equivalent ? "equivalent" : "superseding",
        current,
        equivalent ? "exactEquivalent" : "sameTarget",
        false,
      );
    }
  }
  ...
  const related = input.constraining.find((decision) => {
    ...
    return (
      runtimeId !== undefined &&
      runtimeId !== targetId &&
      input.friction.affectedBlockIds.includes(runtimeId)
    );
  });
...
function prospectiveCandidate(input: Parameters<typeof classifyFix>[0]) {
  try {
    const applied = applySuggestedFix({
      frictionPoints: input.frictionPoints.map(cloneFriction),
      ...
      selectedFrictionPointId: input.friction.id,
      selectedSuggestedFixId: input.fix.id,
      dayBoundaryStartTime: input.dayBoundaryStartTime,
      revisedAt: input.classifiedAt,
    ...
    return createPlanDecisionAcceptanceCandidate({
      suggestedFix: input.fix,
      frictionPoint: input.friction,
...
function preview(
  input: Parameters<typeof classifyFix>[0],
  scheduledBlocks: readonly DraftScheduledBlock[],
  unplacedCandidates: readonly BlockCandidate[],
) {
  return {
    generatedWorkBlocks: [...input.generatedWorkBlocks],
    blockCandidates: [...input.blockCandidates],
    scheduledBlocks: [...scheduledBlocks],
    unplacedCandidates: [...unplacedCandidates],
    frictionPoints: [...input.frictionPoints],
    planDecisionResults: [...input.replayResults],
  };
}

function result(
  input: Parameters<typeof classifyFix>[0],
  relationship: SuggestedFixDecisionContext["relationship"],
  decision: PlanDecisionV1 | undefined,
  explanationCode: SuggestedFixDecisionContext["explanationCode"] | undefined,
  hasBlockedRelation: boolean,
) {
  const replay = decision ? input.replayById.get(decision.id) : undefined;
  const context: SuggestedFixDecisionContext = {
    relationship,
    ...(decision ? { decisionId: decision.id } : {}),
    ...(replay ? { replayStatus: replay.status } : {}),
    ...(explanationCode ? { explanationCode } : {}),
  };
  return { fix: { ...input.fix }, context, originalIndex: input.originalIndex, hasBlockedRelation };
}

function buildRuntimeTargetKeys(
  input: Parameters<typeof classifySuggestedFixes>[0],
...
function getFixTargetId(fix: SuggestedFix, friction: FrictionPoint): string | null {
  ...
  for (const prefix of prefixes)
    if (fix.id.startsWith(prefix)) {
      const id = fix.id.slice(prefix.length);
      if (friction.affectedBlockIds.includes(id)) return id;
    }
...
function rank(value: SuggestedFixDecisionContext["relationship"], blocked: boolean) {
  const tiers = blocked
    ? { unblocking: 0, preserving: 1, ordinary: 2, unknown: 2, superseding: 3, equivalent: 4 }
...
function cloneFriction(value: FrictionPoint): FrictionPoint {
  return {
    ...value,
    affectedBlockIds: [...value.affectedBlockIds],
    suggestedFixes: value.suggestedFixes.map((fix) => ({
      ...fix,
