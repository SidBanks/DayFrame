...
function allocateFrom(start: number) {
  let value = start;
  return () =>
    `00000000-0000-4000-8000-${String(value++).padStart(12, "0")}` as SourceIncarnationId;
}
...
describe("source incarnation scheduling non-interference", () => {
  it("does not affect scheduling or OccurrenceIdentity V1", () => {
    ...
    const generate = (active: typeof first) =>
    ...
    expect(secondResult.blockCandidates[0]?.commitmentNavigationIdentity).not.toEqual(
      firstResult.blockCandidates[0]?.commitmentNavigationIdentity,
    );
    expect(withoutNavigationProvenance(secondResult)).toEqual(
      withoutNavigationProvenance(firstResult),
    );
...
function withoutNavigationProvenance<T>(value: T): T {
  const clone = structuredClone(value);
  if (!clone || typeof clone !== "object") return clone;
  const result = clone as {
    blockCandidates?: Array<{ commitmentNavigationIdentity?: unknown }>;
    scheduledBlocks?: Array<{ commitmentNavigationIdentity?: unknown }>;
    unplacedCandidates?: Array<{ commitmentNavigationIdentity?: unknown }>;
  };
  for (const item of [
    ...(result.blockCandidates ?? []),
    ...(result.scheduledBlocks ?? []),
    ...(result.unplacedCandidates ?? []),
  ]) {
    delete item.commitmentNavigationIdentity;
  }
  return clone;
}
