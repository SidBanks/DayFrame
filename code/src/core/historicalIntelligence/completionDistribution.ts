...
export type CompletionClassificationV1 =
  | "completed"
  | "partial"
  | "skipped"
  | "unknown"
...
export function validateHistoricalCompletionDistributionQuery(
  ...
  if (
    query.policy.id !== HISTORICAL_METRIC_POLICY_V1.id ||
    query.policy.version !== HISTORICAL_METRIC_POLICY_V1.version
  )
    issues.push("unsupportedPolicy");
  ...
  if (
    validLocalDate(query.startUserDayDate) &&
    validLocalDate(query.endUserDayDate) &&
    query.startUserDayDate > query.endUserDayDate
  )
...
export function projectHistoricalCompletionDistributionV1(input: {
    ...
    for (const occurrence of evidence.day.occurrences) {
      ...
      if (occurrence.plan.state !== "scheduled") {
        excluded.push({
          ...explanation,
          reason:
            occurrence.plan.state === "unplaced"
              ? "excludedUnplaced"
              : occurrence.plan.state === "omitted"
                ? "excludedOmitted"
                : "excludedBlocked",
        });
        continue;
      ...
      const classification: CompletionClassificationV1 = !item
        ? "notReported"
  ...
  const distribution: CompletionDistributionV1 = {
    ...counts,
  ...
  const currentOutcomeCoverage: CurrentOutcomeCoverageV1 = unavailable
    ...
    : zero
      ? { status: "notApplicable", classifiedCount: 0, eligibleCount: 0 }
      : {
          status: "available",
          classifiedCount: counts.completed + counts.partial + counts.skipped,
  ...
  return structuredClone({
    status: "projected",
    metric: COMPLETION_DISTRIBUTION_METRIC_V1,
    ...
    limitations,
    provenance: { eligible, excluded, missingPlanUserDayDates: missing },
  });
}

function explain(
  evidence: EffectiveHistoricalPlanDayEvidence,
  occurrence: HistoricalPlanDayPublicationV1["occurrences"][number],
): HistoricalOccurrenceExplanationV1 {
  return {
...
function compareProvenance(
  left: HistoricalOccurrenceExplanationV1,
  right: HistoricalOccurrenceExplanationV1,
): number {
  const start = (value: HistoricalOccurrenceExplanationV1) =>
