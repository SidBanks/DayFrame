...
export function resolveHistoricalPlanCoverageV1(input: {
  ...
  const missing = [...new Set(input.missingUserDayDates.filter((date) => expected.has(date)))]
    .sort() as LocalDateString[];
  const missingSet = new Set(missing);
  const days = input.days.filter((value) => expected.has(value.day.userDayDate) &&
      value.publishedAt <= input.evaluationAsOf && !missingSet.has(value.day.userDayDate))
  ...
  return { coverage: {
    status: publishedDayCount === 0 ? "unavailable"
      : missing.length === 0 ? "complete" : "incompleteCoverage",
...
function enumerateDates(start: LocalDateString, end: LocalDateString): LocalDateString[] {
  ...
  for (let at = Date.parse(`${start}T00:00:00.000Z`); at <= Date.parse(`${end}T00:00:00.000Z`);
    at += 86_400_000) dates.push(new Date(at).toISOString().slice(0, 10) as LocalDateString);
  return dates;
}
