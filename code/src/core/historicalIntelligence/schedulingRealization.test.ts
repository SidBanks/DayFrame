...
} from "./schedulingRealization.js";

const inc = (value: string) => value as SourceIncarnationId;
const query: HistoricalSchedulingRealizationQueryV1 = {
...
const occurrence = (slot: number, plan: HistoricalPlanContext) => ({
  version: 1 as const,
...
describe("Scheduling Realization Projection V1", () => {
  ...
  it("classifies all four HistoricalPlan dispositions exactly once and conserves the denominator", () => {
    ...
    expect(result).toMatchObject({
      status: "projected",
      metric: { id: "schedulingRealization", version: 1 },
      policy: HISTORICAL_METRIC_POLICY_V1,
      planCoverage: {
        status: "complete",
        expectedDayCount: 2,
    ...
    if (result.status === "projected")
      expect(result.distribution.intendedOccurrenceCount).toBe(
        result.distribution.scheduled +
          result.distribution.unplaced +
          result.distribution.omitted +
  ...
  it("distinguishes published-empty, incomplete, and unavailable coverage", () => {
    ...
    expect(
      project([day("2026-08-20", [occurrence(0, { state: "unplaced" })])], ["2026-08-21"]),
    ).toMatchObject({
      status: "projected",
      planCoverage: { status: "incompleteCoverage" },
  ...
  it("preserves exact incarnations and frozen state-specific provenance without inventing reasons", () => {
    ...
    const recreated = {
      ...occurrence(0, { state: "blocked" }),
      reference: reference(0, "cccccccc-cccc-4ccc-8ccc-cccccccccccc"),
    };
    const result = project([day("2026-08-20", [recreated, old]), day("2026-08-21")]);
    expect(result).toMatchObject({
      status: "projected",
      distribution: { intendedOccurrenceCount: 2, blocked: 2 },
      provenance: {
        occurrences: [
          { planningDisposition: "blocked", plan: { state: "blocked" } },
          { planningDisposition: "blocked", plan: { state: "blocked" } },
        ],
      },
    });
    ...
    expect(
      new Set(
        result.provenance.occurrences.map((value) =>
          value.reference.sourceKind === "template" ? value.reference.template.incarnationId : "",
        ),
      ).size,
    ).toBe(2);
  ...
  it("is deterministic, clone-isolated, and reproducible after durable JSON authority roundtrip", () => {
    ...
    const first = project(days);
    const second = project([...days].reverse());
  ...
  it("has no current Active, Preview, ExecutionHistory, or completion input", () => {
    const input = {
      query,
      days: [day("2026-08-20", [occurrence(0, { state: "unplaced" })]), day("2026-08-21")],
      missingUserDayDates: [],
    };
    const baseline = projectHistoricalSchedulingRealizationV1(input);
    const unrelatedRuntimeAuthorities = {
      active: { revision: 99 },
      preview: { id: "draft" },
      executionHistory: [{ outcome: "completed" }],
      completionProjection: { completed: 500 },
    };
