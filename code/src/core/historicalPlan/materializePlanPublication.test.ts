...
const daily = createTemplateOccurrenceIdentity({
  templateId: "template",
  recurrenceId: "daily",
  frequency: "daily",
...
const weekly = createTemplateOccurrenceIdentity({
  templateId: "template",
  recurrenceId: "weekly",
  frequency: "weekly",
  userDayDate: "2026-08-20",
...
const authored = {
  ...
  previewRange: { preset: "custom", startDate: "2026-08-20", endDate: "2026-08-21" },
  shiftDefinitions: [],
  shiftCycles: [],
  blockTemplates: [
    {
      id: "template",
      incarnationId: inc("000000000001"),
      userId: "u",
      title: "Workout",
      category: "fitness",
  ...
  blockRecurrences: [
    {
      id: "daily",
      incarnationId: inc("000000000002"),
      blockTemplateId: "template",
      frequency: "daily",
    },
    {
      id: "weekly",
      incarnationId: inc("000000000003"),
      blockTemplateId: "template",
      frequency: "weekly",
    },
  ],
  manualEvents: [
    {
      id: "manual",
      incarnationId: inc("000000000004"),
      title: "Appointment",
      userDayDate: "2026-08-20",
      allDay: false,
      startTime: "20:00",
...
const candidate = (identity = daily) => ({
  id: `candidate-${identity.slot}-${identity.frequency}`,
  occurrenceIdentity: identity,
  userId: "u",
  templateId: "template",
  recurrenceId: identity.recurrenceId,
  recurrenceFrequency: identity.frequency,
  title: "Workout",
  category: "fitness",
  placementType: "flexible",
  durationMinutes: 60,
...
const scheduled = (identity = daily, id = "scheduled"): DraftScheduledBlock =>
  ({
    ...candidate(identity),
    id,
    source: "template",
...
function preview(overrides: Partial<DayFramePreview> = {}): DayFramePreview {
  return {
    ...
    rangeEndDate: "2026-08-21",
    planningWindowStart: new Date("2026-08-20T03:00:00Z"),
    planningWindowEnd: new Date("2026-08-22T03:00:00Z"),
...
describe("HistoricalPlan publication materialization", () => {
  it("rejects no-preview, stale, and Try authority", () => {
    expect(materializePlanPublication({ authoredSetup: authored, preview: null }).status).toBe(
      "noPreview",
    );
    expect(
      materializePlanPublication({ authoredSetup: authored, preview: preview({ isStale: true }) })
        .status,
    ).toBe("stalePreview");
    expect(
      materializePlanPublication({
        authoredSetup: authored,
        preview: preview({ revisedAt: "2026-08-20T12:00:01.000Z" }),
      }).status,
    ).toBe("tryPreview");
  });
  it("publishes exact requested range with explicit empty days and overnight geometry", () => {
    ...
    expect(result.batch.days).toHaveLength(2);
    expect(result.batch.days[1]!.occurrences).toEqual([]);
    expect(result.batch.days[0]!.occurrences[0]!.goals).toEqual([]);
    expect(result.batch.days[0]!.occurrences[0]).toMatchObject({
      version: 2,
      timing: { kind: "timed" },
    });
    expect(result.batch.days[0]!.occurrences[0]!.plan).toEqual({
      state: "scheduled",
  ...
  it("assigns manual and user-week reference forms to fresh Preview containing day", () => {
    ...
    expect(result.status).toBe("materialized");
    if (result.status !== "materialized") return;
    expect(result.batch.days[0]!.occurrences.map((value) => value.sourceFamily).sort()).toEqual([
      "manualEvent",
      "template",
    ]);
    expect(
      result.batch.days[0]!.occurrences.every(
        (value) => value.version === 2 && value.timing.kind === "timed",
      ),
    ).toBe(true);
  });
  it("freezes manual all-day intent independently from identical timed geometry", () => {
    const current = preview();
    const manualTimed = {
      ...scheduled(daily, "manual-timed"),
      occurrenceIdentity: manual,
      source: "manual",
      title: "Appointment",
      category: "optional",
    ...
    const manualAllDay = { ...manualTimed, id: "manual-all-day", isAllDay: true };
    current.result.scheduledBlocks = [manualAllDay];
    let result = materializePlanPublication({
      authoredSetup: authored,
      preview: current,
      providers,
    });
    expect(result.status === "materialized" && result.batch.days[0]!.occurrences[0]).toMatchObject({
      version: 2,
      timing: { kind: "allDay" },
    });
    current.result.scheduledBlocks = [manualTimed];
  ...
  it("materializes unplaced, blocked, and omitted state without intervals", () => {
    const current = preview();
    current.result.scheduledBlocks = [];
    current.result.unplacedCandidates = [candidate() as never];
    const reference = createDurableOccurrenceReference(daily, authored);
    if (reference.status !== "created") throw new Error("fixture");
    ...
    expect(result.status === "materialized" && result.batch.days[0]!.occurrences[0]!.plan).toEqual({
      state: "blocked",
    });
    const omitId = "30000000-0000-4000-8000-000000000002" as PlanDecisionId;
    current.result.unplacedCandidates = [];
    current.result.planDecisionResults = [
      {
        decisionId: omitId,
        kind: "omitOccurrence",
  ...
  it("excludes hidden imported/synthetic items without adding unsupported identity", () => {
    ...
    expect(result.status).toBe("materialized");
    if (result.status === "materialized") expect(result.batch.days[0]!.occurrences).toHaveLength(1);
  });
  it("freezes exact linked Goal provenance without later relabeling", () => {
    const goal = {
      version: 1 as const,
    ...
    goal.title = "Renamed";
    expect(result.batch.days[0]!.occurrences[0]!.goals?.[0]?.title).toBe("Earn Security+");
  });
});
