...
function created(
  identity: Parameters<typeof createDurableOccurrenceReference>[0],
...
describe("DurableOccurrenceReference V1", () => {
  it("constructs and resolves template, overnight work, and manual families", () => {
    ...
    expect(references.map((reference) => reference.sourceKind)).toEqual([
      "template",
      "work",
      "manualEvent",
    ]);
  ...
  it("is stable across construction, JSON roundtrip, regeneration, and clone boundaries", () => {
    ...
    if (validation.status === "valid") {
      ...
      if (validation.reference.sourceKind === "template")
        validation.reference.template.id = "changed";
    }
  ...
  it("strictly rejects malformed, cross-family, invalid date/slot, and future-version input", () => {
    ...
    expect(validateDurableOccurrenceReference({ ...reference, unexpected: true }).status).toBe(
      "invalid",
    );
    expect(validateDurableOccurrenceReference({ ...reference, version: 2 }).status).toBe(
      "unsupportedVersion",
    );
    expect(
      validateDurableOccurrenceReference({
    ...
    expect(
      validateDurableOccurrenceReference({
        ...
        manualEvent: (reference as { template: unknown }).template,
        coordinate: {},
      }).status,
    ).toBe("invalid");
  });
  ...
  it("distinguishes source deletion, lifetime mismatch, and occurrence removal", () => {
    ...
    const updated = {
      ...state,
      blockRecurrences: state.blockRecurrences.map((source) => ({
        ...source,
        startsOnDate: "2026-08-21" as const,
      })),
    };
  ...
  it("diagnoses each work lineage component and nested parent recreation", () => {
    ...
    const entryChanged = {
      ...state,
      shiftCycles: state.shiftCycles.map((cycle) => ({
        ...cycle,
        segments: cycle.segments.map((entry) => ({
          ...entry,
    ...
    const shiftChanged = {
      ...state,
      shiftDefinitions: state.shiftDefinitions.map((shift) => ({
        ...shift,
        incarnationId: newGraph.shiftDefinitions[0]!.incarnationId,
      })),
    };
  ...
  it("keeps manual references resolved after an ordinary move", () => {
    ...
    const moved = {
      ...
      manualEvents: state.manualEvents.map((event) => ({
        ...event,
        userDayDate: "2026-09-01" as const,
        title: "Moved",
      })),
    };
  ...
  it("returns explicit construction failures instead of fabricating lineage", () => {
    const state = active();
    expect(
      createDurableOccurrenceReference(createManualEventOccurrenceIdentity("missing"), state),
