import { projectOccurrenceOutcome, type ExecutionAssertionRecordV1, type ExecutionRecordV1,
  type ExecutionSubjectId, type OccurrenceOutcome } from "./executionRecord.js";
...
export function buildExecutionHistoryItems(records: readonly ExecutionRecordV1[]): ExecutionHistoryItem[] {
  ...
  for (const [subjectId, subjectRecords] of grouped) {
    ...
    const latestAssertion = [...revisions].reverse().find((record): record is ExecutionAssertionRecordV1 => record.kind === "assertion");
    ...
    items.push({ subjectId, currentOutcome: structuredClone(currentOutcome), currentRecord: structuredClone(currentRecord),
      snapshot: structuredClone(latestAssertion.snapshot), subject: structuredClone(latestAssertion.subject),
      revisions: revisions.map((record) => structuredClone(record)) });
  }
  return items.sort((left, right) => right.snapshot.userDay.date.localeCompare(left.snapshot.userDay.date) ||
    plannedStart(right).localeCompare(plannedStart(left)) || left.subjectId.localeCompare(right.subjectId));
}

export function orderSubjectChain(records: readonly ExecutionRecordV1[]): ExecutionRecordV1[] {
  if (records.length === 0) return [];
  const byReplacement = new Map(records.filter((record) => record.replacesRecordId)
  ...
  while (byReplacement.has(ordered.at(-1)!.id)) ordered.push(byReplacement.get(ordered.at(-1)!.id)!);
  return ordered;
}
