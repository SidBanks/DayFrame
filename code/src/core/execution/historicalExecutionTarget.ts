...
import { createDurableOccurrenceReference, durableOccurrenceReferencesEqual,
  resolveDurableOccurrenceReference, validateDurableOccurrenceReference,
  type DurableOccurrenceReference } from "../occurrences/durableOccurrenceReference.js";
...
import { validateExecutionHistoricalSnapshot, type ExecutionHistoricalSnapshot } from "./executionRecord.js";
...
export function materializeHistoricalExecutionTarget(input: {
  ...
  switch (input.selection.kind) {
    case "scheduledBlock": {
      ...
      if (block.source === "importedCalendar" || block.source === "rule" || !block.occurrenceIdentity) return { status: "unsupportedFamily" };
      const previewCandidate = candidateForIdentity(preview.result.blockCandidates, block.occurrenceIdentity, input.authoredSetup);
      return fromOccurrence(block.occurrenceIdentity, input.authoredSetup, {
        plan: { state: "scheduled", startsAt: block.startsAt.toISOString(), endsAt: block.endsAt.toISOString() },
    ...
    case "workBlock": {
      ...
      if (!block?.occurrenceIdentity) return block ? { status: "unsupportedFamily" } : { status: "notReportable" };
      return fromOccurrence(block.occurrenceIdentity, input.authoredSetup, {
        plan: { state: "scheduled", startsAt: block.startsAt.toISOString(), endsAt: block.endsAt.toISOString() },
        userDayDate: block.userDayDate,
        placementInstant: block.startsAt,
    ...
    case "unplacedCandidate": {
      ...
      const candidate = preview.result.unplacedCandidates.find((current) => current.id === candidateId);
      if (!candidate?.occurrenceIdentity) return candidate ? { status: "unsupportedFamily" } : { status: "notReportable" };
      const constructed = createDurableOccurrenceReference(candidate.occurrenceIdentity, input.authoredSetup);
      if (constructed.status !== "created") return constructionFailure(constructed.status);
      const blocked = preview.result.planDecisionResults.some((result) => result.status === "blocked" &&
      ...
      return fromOccurrence(candidate.occurrenceIdentity, input.authoredSetup, {
        plan: { state: blocked ? "blocked" : "unplaced" }, userDayDate: candidate.userDayDate,
        placementInstant: userDayNoon(candidate.userDayDate), previewCandidate: candidate,
      });
    }
    case "planDecision":
      return fromDecision(input.selection.decisionId, preview.result.planDecisionResults,
        preview.result.blockCandidates, input.authoredSetup);
  }
}
...
function fromDecision(
  ...
  const state = replay.kind === "omitOccurrence" && replay.status === "applied" ? "omitted" as const
    : replay.kind === "placeOccurrence" && replay.status === "blocked" ? "blocked" as const : undefined;
  ...
  return fromOccurrence(candidate.occurrenceIdentity, authoredSetup, {
    plan: { state }, userDayDate: candidate.userDayDate,
    placementInstant: userDayNoon(candidate.userDayDate), previewCandidate: candidate,
  });
}
...
function materializeHistoricalReference(
  ...
  if (validation.status === "unsupportedVersion") return { status: "invalidReference", issues: ["unsupported reference version"] };
  const resolution = resolveDurableOccurrenceReference(validation.reference, authoredSetup);
  if (resolution.status !== "resolved") return mapResolution(resolution.status, resolution.status === "invalidReference" ? resolution.issues : undefined);
...
function fromOccurrence(
  ...
  authoredSetup: DayFrameAuthoredSetup,
  context: { plan: ExecutionHistoricalSnapshot["plan"]; userDayDate: string;
    placementInstant: Date; previewCandidate?: BlockCandidate },
): HistoricalExecutionTargetMaterialization {
  ...
  if (referenceValidation.status !== "valid") return { status: "invalidReference",
    issues: referenceValidation.status === "invalid" ? referenceValidation.issues : ["unsupported reference version"] };
  ...
  if (snapshotValidation.status === "invalid") return { status: "invalidSnapshot", issues: snapshotValidation.issues };
  return { status: "materialized", target: {
    reference: structuredClone(referenceValidation.reference), snapshot: snapshotValidation.snapshot,
  } };
}

function sourceContext(identity: OccurrenceIdentity, state: DayFrameAuthoredSetup,
  candidate?: BlockCandidate): { title: string; category: ExecutionHistoricalSnapshot["category"] } | undefined {
  if (identity.sourceKind === "template") {
    const template = state.blockTemplates.find((current) => current.id === identity.templateId);
    return template ? { title: candidate?.title ?? template.title, category: candidate?.category ?? template.category } : undefined;
  }
  if (identity.sourceKind === "work") {
    const shift = state.shiftDefinitions.find((current) => current.id === identity.shiftDefinitionId);
...
function candidateForIdentity(candidates: readonly BlockCandidate[], identity: OccurrenceIdentity,
  state: DayFrameAuthoredSetup): BlockCandidate | undefined {
  const constructed = createDurableOccurrenceReference(identity, state);
  return constructed.status === "created" ? candidateForReference(candidates, constructed.reference, state) : undefined;
}

function candidateForReference(candidates: readonly BlockCandidate[], reference: DurableOccurrenceReference,
  state: DayFrameAuthoredSetup): BlockCandidate | undefined {
  return candidates.find((candidate) => {
    ...
    return constructed.status === "created" && durableOccurrenceReferencesEqual(constructed.reference, reference);
  });
}

function resolutionFailure(reference: DurableOccurrenceReference,
  state: DayFrameAuthoredSetup): HistoricalExecutionTargetMaterialization {
  const resolution = resolveDurableOccurrenceReference(reference, state);
  return resolution.status === "resolved" ? { status: "insufficientHistoricalContext" }
    : mapResolution(resolution.status, resolution.status === "invalidReference" ? resolution.issues : undefined);
}

function mapResolution(status: Exclude<ReturnType<typeof resolveDurableOccurrenceReference>["status"], "resolved">,
  issues?: string[]): HistoricalExecutionTargetMaterialization {
...
function constructionFailure(status: "unsupportedOccurrence" | "missingSourceLineage"):
  HistoricalExecutionTargetMaterialization {
  return status === "unsupportedOccurrence" ? { status: "unsupportedFamily" } : { status: "sourceMissing" };
}

function userDayNoon(date: string): Date { return new Date(`${date}T12:00:00`); }

export function isReportableScheduledBlock(block: DraftScheduledBlock): boolean {
  return (block.source === "template" || block.source === "manual") && block.occurrenceIdentity !== undefined;
}
