...
function allocateUuid(name: string): string {
  ...
  if (!UUID_V4.test(value))
    throw new Error(`Cryptographic UUID generation returned an invalid ${name}.`);
  return value;
}
...
export function validateExecutionHistoricalSnapshot(
  value: unknown,
): ExecutionHistoricalSnapshotValidation {
  const issues: string[] = [];
...
export function validateExecutionRecordCollection(
  ...
  values.forEach((value, recordIndex) => {
    ...
    if (result.status === "valid") records.push(result.record);
      ...
      issues.push({
        code: result.status === "unsupportedVersion" ? "unsupportedVersion" : "invalidRecord",
        recordIndex,
        ...(record(value) && typeof value.id === "string" ? { recordId: value.id } : {}),
        ...(record(value) && typeof value.subjectId === "string"
  ...
  return issues.length > 0
    ? { status: "invalid", issues }
    : { status: "valid", records: records.map(cloneExecutionRecord) };
}
...
export function projectCurrentExecutionRecord(
  ...
  const replaced = new Set(
    validation.records.flatMap((current) =>
...
export function projectOccurrenceOutcome(
  ...
  return {
    status: projection.record.outcome,
    record: cloneExecutionRecord(projection.record) as ExecutionAssertionRecordV1,
  };
}

export function cloneExecutionRecord(recordValue: ExecutionRecordV1): ExecutionRecordV1 {
  if (recordValue.kind === "retraction")
    return { ...recordValue, provenance: { ...recordValue.provenance } };
  return {
    ...recordValue,
...
function validateAssertion(value: Record<string, unknown>, issues: string[]): void {
  exact(
    ...
    ["actualTime", "note", "replacesRecordId"],
    "record",
    issues,
  );
...
function validateRetraction(value: Record<string, unknown>, issues: string[]): void {
  exact(
    value,
    ["version", "id", "subjectId", "kind", "provenance", "recordedAt", "replacesRecordId"],
    ["note"],
    "record",
    issues,
  );
  validateBase(value, issues);
  if (!isExecutionRecordId(value.replacesRecordId))
...
function validateBase(value: Record<string, unknown>, issues: string[]): void {
  ...
  if (
    ...
    value.provenance.kind !== "userReported" ||
    Object.keys(value.provenance).length !== 1
  )
    issues.push("provenance must be userReported");
  if (
    value.note !== undefined &&
    (typeof value.note !== "string" || value.note.length > EXECUTION_NOTE_MAX_LENGTH)
  ) {
...
function validateSnapshot(value: unknown, issues: string[]): void {
  ...
  if (
    ...
    value.title.length > EXECUTION_SNAPSHOT_TITLE_MAX_LENGTH
  ) {
    issues.push(
      `snapshot.title must be non-empty and at most ${EXECUTION_SNAPSHOT_TITLE_MAX_LENGTH} characters`,
    );
  }
  if (!BLOCK_CATEGORIES.includes(value.category as BlockCategory))
    issues.push("snapshot.category is invalid");
  if (!record(value.userDay)) issues.push("snapshot.userDay must be an object");
  else {
    exact(
      value.userDay,
      ["date", "dayBoundaryStartTime", "utcOffsetMinutes"],
      [],
      "snapshot.userDay",
      issues,
    );
    ...
    if (
      typeof value.userDay.dayBoundaryStartTime !== "string" ||
      !TIME.test(value.userDay.dayBoundaryStartTime)
    )
      issues.push("snapshot.userDay.dayBoundaryStartTime is invalid");
    if (
      !Number.isInteger(value.userDay.utcOffsetMinutes) ||
      (value.userDay.utcOffsetMinutes as number) < -840 ||
      (value.userDay.utcOffsetMinutes as number) > 840
    )
      issues.push("snapshot.userDay.utcOffsetMinutes must be -840..840");
  }
...
function validatePlan(value: unknown, issues: string[]): void {
  if (!record(value)) {
    issues.push("snapshot.plan must be an object");
    return;
  }
  if (value.state === "scheduled") {
    exact(value, ["state", "startsAt", "endsAt"], [], "snapshot.plan", issues);
    if (!isCanonicalUtc(value.startsAt) || !isCanonicalUtc(value.endsAt))
      issues.push("scheduled plan interval must use canonical UTC instants");
    else if (Date.parse(value.endsAt as string) <= Date.parse(value.startsAt as string))
      issues.push("scheduled plan end must be after start");
...
function validateActualTime(
  ...
  if (!record(value)) {
    issues.push("actualTime must be an object");
    return;
  }
  ...
  if (value.occurredAt !== undefined) {
    if (!isCanonicalUtc(value.occurredAt))
      issues.push("actualTime.occurredAt must be canonical UTC ISO-8601");
    else if (isCanonicalUtc(recordedAt) && Date.parse(value.occurredAt) > Date.parse(recordedAt))
      issues.push("actualTime.occurredAt must not be after recordedAt");
  }
  if (
    value.durationMinutes !== undefined &&
    (!Number.isInteger(value.durationMinutes) ||
      (value.durationMinutes as number) < 1 ||
      (value.durationMinutes as number) > EXECUTION_ACTUAL_DURATION_MAX_MINUTES)
  ) {
    issues.push(`actualTime.durationMinutes must be 1..${EXECUTION_ACTUAL_DURATION_MAX_MINUTES}`);
  }
}

function validateSubjectSnapshot(
  ...
  if (subject.kind === "unplanned") {
    if (
      ...
      (record(snapshot.plan) && snapshot.plan.state !== "unplanned")
    )
      issues.push("unplanned subject requires unplanned snapshot and plan state");
    return;
  }
  if (subject.kind === "planned" && record(subject.reference)) {
    if (snapshot.sourceFamily !== subject.reference.sourceKind)
...
function validateCollectionIntegrity(records: ExecutionRecordV1[]): ExecutionCollectionIssue[] {
  ...
  for (const current of records) {
    ...
    if (!target) {
      issues.push(issue("missingReplacement", current));
      continue;
    }
    if (target.subjectId !== current.subjectId)
      issues.push(issue("crossSubjectReplacement", current));
    if (Date.parse(current.recordedAt) < Date.parse(target.recordedAt))
      issues.push(issue("nonMonotonicRecordedAt", current));
  ...
  for (const replacements of replacedBy.values()) {
    if (replacements.length > 1)
      for (const current of replacements) issues.push(issue("competingReplacement", current));
  }
  ...
  for (const [subjectId, subjectRecords] of bySubject) {
    ...
    if (origin?.kind === "planned")
      plannedSubjects.push({ subjectId, reference: origin.reference });
  ...
  for (let left = 0; left < plannedSubjects.length; left += 1) {
    for (let right = left + 1; right < plannedSubjects.length; right += 1) {
      if (
        durableOccurrenceReferencesEqual(
          plannedSubjects[left]!.reference,
          plannedSubjects[right]!.reference,
        )
      ) {
        issues.push({
          code: "duplicatePlannedReference",
          subjectId: plannedSubjects[right]!.subjectId,
        });
      }
    }
  }
...
function hasCycle(
  subjectRecords: ExecutionRecordV1[],
  byId: Map<string, ExecutionRecordV1>,
): boolean {
...
function replacementContext(
  ...
  if (validation.status === "invalid")
    return { status: "invalidReplacement", issues: validation.issues };
  const target = validation.records.find((current) => current.id === targetId);
  if (!target)
    return {
      status: "invalidReplacement",
      issues: [{ code: "missingReplacement", recordId: targetId }],
    };
  const projectionHead = findHead(
    validation.records.filter((current) => current.subjectId === target.subjectId),
  );
  if (!projectionHead || projectionHead.id !== target.id) return { status: "notCurrentHead" };
  const assertion = [...validation.records]
    .reverse()
    .find((current) => current.subjectId === target.subjectId && current.kind === "assertion") as
    | ExecutionAssertionRecordV1
    | undefined;
  if (!assertion)
    return {
      status: "invalidReplacement",
      issues: [{ code: "subjectMismatch", subjectId: target.subjectId }],
    };
...
function validatedReplacement(
  ...
  if (validation.status !== "valid")
    return {
      status: "invalidInput",
      issues: validation.status === "invalid" ? validation.issues : ["unsupported version"],
    };
  const collection = validateExecutionRecordCollection([...records, validation.record]);
  return collection.status === "valid"
    ? { status: "created", record: validation.record }
...
function findHead(records: ExecutionRecordV1[]): ExecutionRecordV1 | undefined {
  const replaced = new Set(
    records.flatMap((current) => (current.replacesRecordId ? [current.replacesRecordId] : [])),
  );
...
function groupBySubject(records: ExecutionRecordV1[]): Map<string, ExecutionRecordV1[]> {
  ...
  for (const current of records)
    groups.set(current.subjectId, [...(groups.get(current.subjectId) ?? []), current]);
  return groups;
}
