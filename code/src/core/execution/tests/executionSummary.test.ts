...
function preview(overrides: Partial<DayFramePreview> = {}): DayFramePreview {
  return {
    ...
    planningWindowEnd: new Date("2026-08-28T03:00:00Z"),
...
function record(
  ...
  return {
    version: 1,
    id: uuid(subject),
    subjectId: uuid(`1${subject.slice(1)}`),
    kind: "assertion",
    ...
    ...(replaces ? { replacesRecordId: uuid(replaces) } : {}),
...
describe("execution summary", () => {
  it("counts each current subject once, scopes by frozen user-day, and reclassifies corrections/retractions", () => {
    const plannedTarget = materializeHistoricalExecutionTarget({
      authoredSetup: state,
      preview: preview(),
    ...
    const skipped = {
      ...record("000000000003", "skipped", "2026-08-22"),
      subject: { kind: "planned", reference: plannedTarget.target.reference },
      snapshot: plannedTarget.target.snapshot,
    } as ExecutionRecordV1;
    const corrected = {
      ...record("000000000004", "partial", "2026-08-20"),
      subjectId: completed.subjectId,
      replacesRecordId: completed.id,
    } as ExecutionRecordV1;
    const retracted = {
      version: 1,
      ...
      replacesRecordId: skipped.id,
    } as unknown as ExecutionRecordV1;
    const result = deriveOutcomeSummary({
      records: [retracted, partial, corrected, skipped, completed],
    ...
    expect(result).toMatchObject({
      ...
      summary: {
        ...
        partial: 2,
        skipped: 0,
        knownNotReportedSubjects: 1,
        totalSubjects: 3,
        bySourceFamily: { unplanned: { partial: 2 }, template: { knownNotReportedSubjects: 1 } },
      },
    });
  });
  ...
  it("returns explicit invalid range and zeros for empty history", () => {
    expect(
      deriveOutcomeSummary({
        records: [],
        startUserDayDate: "2026-08-22",
        endUserDayDate: "2026-08-20",
      }),
    ).toEqual({ status: "invalidRange" });
    expect(deriveOutcomeSummary({ records: [] })).toMatchObject({
      status: "available",
  ...
  it("derives fresh coverage, excludes unsupported imported blocks, and is runtime-order independent", () => {
    ...
    const imported = {
      ...current.result.scheduledBlocks[0]!,
      id: "imported",
      source: "importedCalendar" as const,
    };
    delete imported.occurrenceIdentity;
    current.result.scheduledBlocks.push(imported);
    const target = materializeHistoricalExecutionTarget({
      authoredSetup: state,
      preview: current,
      selection: { kind: "scheduledBlock", blockId: "scheduled" },
    });
    ...
    const history = {
      ...record("000000000001", "completed", "2026-08-20"),
    ...
    expect(
      ...
      status: "available",
      coverage: { eligibleOccurrences: 3, reportedOccurrences: 1, unreportedOccurrences: 2 },
    });
  ...
  it("deduplicates overlapping representations and treats a retracted subject as unreported", () => {
    ...
    const ref = materializeHistoricalExecutionTarget({
      authoredSetup: state,
      preview: current,
      selection: { kind: "scheduledBlock", blockId: "scheduled" },
    });
    ...
    const assertion = {
      ...record("000000000001", "completed", "2026-08-20"),
      subject: { kind: "planned", reference: ref.target.reference },
      snapshot: ref.target.snapshot,
    } as ExecutionRecordV1;
    const retraction = {
      version: 1,
      ...
      replacesRecordId: assertion.id,
    } as unknown as ExecutionRecordV1;
    expect(
      deriveCurrentPreviewReportingCoverage({
        authoredSetup: state,
        preview: current,
        records: [retraction, assertion],
      }),
    ).toEqual({
      status: "available",
      coverage: { eligibleOccurrences: 3, reportedOccurrences: 0, unreportedOccurrences: 3 },
    });
  });

  it("returns explicit no, stale, and Try Preview availability", () => {
    expect(
      deriveCurrentPreviewReportingCoverage({ authoredSetup: state, preview: null, records: [] }),
    ).toEqual({ status: "unavailable", reason: "noPreview" });
    expect(
      deriveCurrentPreviewReportingCoverage({
        authoredSetup: state,
    ...
    ).toEqual({ status: "unavailable", reason: "stalePreview" });
    expect(
      deriveCurrentPreviewReportingCoverage({
        authoredSetup: state,
        preview: preview({ revisedAt: "2026-08-20T13:00:00Z" }),
        records: [],
      }),
    ).toEqual({ status: "unavailable", reason: "tryPreview" });
  });
  ...
  it("does not join an old lifetime and fails closed for inconsistent current context", () => {
    ...
    if (target.status !== "materialized" || target.target.reference.sourceKind !== "template")
      throw new Error("fixture did not materialize");
    const oldReference = structuredClone(target.target.reference);
    oldReference.template.incarnationId = incarnation("000000000099");
    const oldHistory = {
      ...record("000000000001", "completed", "2026-08-20"),
      subject: { kind: "planned", reference: oldReference },
      snapshot: target.target.snapshot,
    } as ExecutionRecordV1;
    expect(
      deriveCurrentPreviewReportingCoverage({
      ...
      status: "available",
      coverage: { eligibleOccurrences: 3, reportedOccurrences: 0 },
    });
    const inconsistent = { ...state, blockTemplates: [] };
    expect(
      deriveCurrentPreviewReportingCoverage({
        authoredSetup: inconsistent,
        preview: current,
        records: [],
      }),
