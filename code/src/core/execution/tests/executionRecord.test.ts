...
describe("ExecutionRecord V1 identity and construction", () => {
  ...
  it("creates a planned assertion with injected identities/clock and isolated inputs", () => {
    ...
    const result = createExecutionAssertion(
      {
        subject: { kind: "planned", reference: inputReference },
        snapshot: inputSnapshot,
        outcome: "completed",
        actualTime: { occurredAt: "2026-08-20T17:00:00.000Z", durationMinutes: 45 },
        note: "Felt good",
      },
      providers(),
    );
    ...
    expect(
      result.record.subject.kind === "planned" &&
        result.record.subject.reference.sourceKind === "template" &&
        result.record.subject.reference.template.id,
    ).toBe("template");
  ...
  it("supports distinct unplanned completed/partial subjects without references", () => {
    const unplannedSnapshot: ExecutionHistoricalSnapshot = {
      ...snapshot({ state: "unplanned" }),
      sourceFamily: "unplanned",
      title: "Spontaneous walk",
    };
    for (const outcome of ["completed", "partial"] as const) {
      const result = createExecutionAssertion(
        { subject: { kind: "unplanned" }, snapshot: unplannedSnapshot, outcome },
        providers(),
      );
      expect(result.status).toBe("created");
      if (result.status === "created" && result.record.kind === "assertion") {
    ...
    const result = createExecutionAssertion(
      {
        ...
        snapshot: { ...snapshot({ state: "unplanned" }), sourceFamily: "unplanned" },
        outcome: "skipped",
      },
      providers(),
    );
  ...
  it.each(["omitted", "unplaced", "blocked"] as const)(
    "allows completed planned %s context",
    (state) => {
...
describe("strict record semantics", () => {
      ...
      const result = createExecutionAssertion(
        {
          ...
          snapshot: snapshot(),
          outcome,
          actualTime: { occurredAt: "2020-01-01T00:00:00.000Z", durationMinutes: 30 },
        },
        providers(),
      );
      expect(result.status).toBe("created");
  ...
  it("rejects skipped actual evidence, future actual time, and invalid duration", () => {
    const cases = [
      ...
      { outcome: "completed" as const, actualTime: { occurredAt: "2026-08-20T18:00:00.001Z" } },
      {
        ...
        actualTime: { durationMinutes: EXECUTION_ACTUAL_DURATION_MAX_MINUTES + 1 },
      },
    ];
    for (const current of cases)
      expect(
        createExecutionAssertion(
          { subject: { kind: "planned", reference }, snapshot: snapshot(), ...current },
          providers(),
        ).status,
      ).toBe("invalidInput");
  });
  ...
  it("enforces title, note, user-day, offset, interval, provenance, and exact keys", () => {
    ...
    const invalid = [
      {
        ...base,
        snapshot: { ...base.snapshot, title: "x".repeat(EXECUTION_SNAPSHOT_TITLE_MAX_LENGTH + 1) },
      },
      { ...base, note: "x".repeat(EXECUTION_NOTE_MAX_LENGTH + 1) },
      {
        ...base,
        snapshot: { ...base.snapshot, userDay: { ...base.snapshot.userDay, date: "2026-02-30" } },
      },
      {
        ...base,
        snapshot: {
          ...base.snapshot,
          userDay: { ...base.snapshot.userDay, utcOffsetMinutes: 841 },
        },
      },
      {
        ...base,
  ...
  it("reports unsupported versions explicitly and validates planned reference/family without resolving sources", () => {
    expect(validateExecutionRecord({ ...planned(), version: 2 })).toEqual({
      status: "unsupportedVersion",
      version: 2,
    });
    expect(
      validateExecutionRecord({ ...planned(), snapshot: { ...snapshot(), sourceFamily: "work" } })
        .status,
    ).toBe("invalid");
  ...
  it("distinguishes recreated source incarnations structurally", () => {
    ...
    const recreated = createExecutionAssertion(
      {
        subject: { kind: "planned", reference: recreatedReference },
        snapshot: snapshot(),
        outcome: "completed",
      },
      providers(ids.r2, ids.s2),
    );
...
describe("correction, retraction, and projection", () => {
  it("corrects the current head immutably and deterministically", () => {
    ...
    const corrected = correctExecutionAssertion(
      ...
      first.id,
      {
        snapshot: { ...snapshot(), title: "Corrected" },
        outcome: "partial",
    ...
    expect(corrected.record).toMatchObject({
      ...
      replacesRecordId: first.id,
    });
    expect(first).toEqual(before);
    for (const records of [
      [first, corrected.record],
      [corrected.record, first],
    ]) {
      expect(projectOccurrenceOutcome(records, first.subjectId)).toMatchObject({
        status: "partial",
      });
    }
    expect(
      correctExecutionAssertion(
        [first, corrected.record],
        first.id,
        { snapshot: snapshot(), outcome: "completed" },
        providers(ids.r3),
      ).status,
    ).toBe("notCurrentHead");
  });
  ...
  it("retracts to unknown and permits a later assertion replacing the retraction", () => {
    const first = planned();
    const retracted = retractExecutionRecord(
      [first],
      first.id,
      "wrong entry",
      providers(ids.r2, ids.s2, "2026-08-20T19:00:00.000Z"),
    ...
    expect(projectCurrentExecutionRecord([first, retracted.record], first.subjectId)).toEqual({
      status: "none",
    });
    expect(projectOccurrenceOutcome([retracted.record, first], first.subjectId)).toEqual({
      status: "unknown",
    });
    const restored = correctExecutionAssertion(
      [retracted.record, first],
      retracted.record.id,
      { snapshot: snapshot(), outcome: "partial" },
      providers(ids.r3, ids.s2, "2026-08-20T20:00:00.000Z"),
    );
    expect(restored.status).toBe("created");
    if (restored.status === "created")
      expect(
        projectOccurrenceOutcome([restored.record, first, retracted.record], first.subjectId),
...
describe("collection integrity", () => {
  function correctedPair(): [ExecutionAssertionRecordV1, ExecutionRecordV1] {
    const first = planned();
    const result = correctExecutionAssertion(
      [first],
      first.id,
      { snapshot: snapshot(), outcome: "partial" },
      providers(ids.r2, ids.s2, "2026-08-20T19:00:00.000Z"),
  ...
  it("detects duplicate IDs, missing targets, cross-subject links, nonmonotonic time, and branches", () => {
    ...
    for (const [records, code] of cases) {
      ...
      if (result.status === "invalid")
        expect(result.issues.some((current) => current.code === code)).toBe(true);
    }
  });
  ...
  it("detects cycles and competing disconnected heads", () => {
    ...
    if (cycle.status === "invalid")
      expect(cycle.issues.some((current) => current.code === "cycle")).toBe(true);

    const competing = validateExecutionRecordCollection([planned(), { ...planned(), id: ids.r2 }]);
    expect(competing.status).toBe("invalid");
    if (competing.status === "invalid")
      expect(competing.issues.some((current) => current.code === "competingHead")).toBe(true);
  });
  ...
  it("detects duplicate planned references across subjects while allowing duplicate unplanned context", () => {
    ...
    expect(result.status).toBe("invalid");
    if (result.status === "invalid")
      expect(result.issues.some((current) => current.code === "duplicatePlannedReference")).toBe(
        true,
      );
    ...
    const one = createExecutionAssertion(
      { subject: { kind: "unplanned" }, snapshot: unplannedSnapshot, outcome: "completed" },
      providers(ids.r1, ids.s1),
    );
    const two = createExecutionAssertion(
      { subject: { kind: "unplanned" }, snapshot: unplannedSnapshot, outcome: "completed" },
      providers(ids.r2, ids.s2),
    );
  ...
  it("validates one subject component independently with structured details", () => {
    const [first, second] = correctedPair();
    expect(validateExecutionSubjectComponent([second, first], first.subjectId).status).toBe(
      "valid",
    );
    const invalid = validateExecutionSubjectComponent(
      [{ ...second, replacesRecordId: ids.r4 }, first],
      first.subjectId,
    );
    expect(invalid.status).toBe("invalid");
    if (invalid.status === "invalid")
      expect(invalid.issues[0]).toMatchObject({ subjectId: first.subjectId });
  });
});
...
describe("serialization and purity", () => {
  it("roundtrips assertions, corrections, and retractions through JSON", () => {
    const first = planned();
    const correction = correctExecutionAssertion(
      [first],
      first.id,
      { snapshot: snapshot(), outcome: "partial" },
      providers(ids.r2, ids.s2, "2026-08-20T19:00:00.000Z"),
    ...
    const retraction = retractExecutionRecord(
      [first, correction.record],
    ...
    const parsed = JSON.parse(
      JSON.stringify([first, correction.record, retraction.record]),
    ) as unknown[];
